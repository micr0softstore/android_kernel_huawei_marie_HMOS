...
#define rtnl_unlock()   (0)
#define rtnl_lock()   (0)
#endif

// 2 全局变量定义
extern oal_uint32 band_5g_enabled;
extern oal_int32  g_al_host_init_params[];
...
oal_nl80211_channel_type  g_en_channel_type = NL80211_CHAN_NO_HT;

// 3 函数实现
#ifdef _PRE_WLAN_FEATURE_UAPSD

OAL_STATIC oal_bool_enum_uint8 wal_find_wmm_uapsd(const oal_uint8 *puc_wmm_ie)
{
    /* 判断 WMM UAPSD 是否使能 */
...
oal_uint32 wal_cfg80211_open_wmm(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, const oal_uint8 *puc_param)
{
...
/* 检查RSN IE长度，失败直接返回 */
OAL_STATIC uint32_t wal_check_rsn_remain_len(const uint8_t *ie_data, const uint8_t ie_len,
                                             uint8_t *index, mac_beacon_param_stru *beacon_param)
{
    uint16_t suites_num;
    uint16_t temp;

    beacon_param->bit_del_group_mgmt_cs = OAL_TRUE;
    /* 偏移 RSN 能力信息 */
    *index += 2;
    /* 若不存在PMKID Count (Len = 2), 直接返回 */
    if (MAC_IE_REAMIN_LEN_IS_ENOUGH(2, *index, ie_len, 2) == OAL_FALSE) {
        OAM_WARNING_LOG0(0, OAM_SF_ANY, "{wal_check_rsn_remain_len:: didn't parse grp mgmt cipher}");
        return OAL_SUCC;
    }

    /* 获取PMKID Count */
    suites_num = OAL_MAKE_WORD16(ie_data[*index], ie_data[*index + 1]);
    *index += 2;

    /* PMKID List字段 (16 * PMKID Count) */
    for (temp = 0; temp < suites_num; temp++) {
        /* IE剩余长度不足16 */
        if (MAC_IE_REAMIN_LEN_IS_ENOUGH(2, *index, ie_len, 16) == OAL_FALSE) {
    ...
    if (MAC_IE_REAMIN_LEN_IS_ENOUGH(2, *index, ie_len, 4) == OAL_FALSE) {
        return OAL_SUCC;
    }

    /* 获取Group Management Cipher Suite信息 */
    beacon_param->bit_del_group_mgmt_cs = OAL_FALSE;
    if (ie_data[*index + MAC_OUI_LEN] != WLAN_80211_CIPHER_SUITE_BIP) {
        OAM_WARNING_LOG1(0, OAM_SF_ANY, "{wal_check_rsn_remain_len:: grp_mgmt_policy[%d]}",
            ie_data[*index + MAC_OUI_LEN]);
    }
    return OAL_SUCC;
}

OAL_STATIC uint32_t wal_check_auth_policy(const uint8_t *ie_data, const uint8_t ie_len,
    uint8_t *oui, uint8_t *index, uint8_t *auth_policy)
{
    uint16_t suites_num;
    uint16_t temp;
    ...
    /* 获取认证套件计数 */
    suites_num = OAL_MAKE_WORD16(ie_data[*index], ie_data[*index + 1]);
    suites_num = OAL_MIN(suites_num, WLAN_AUTHENTICATION_SUITES);
    *index += 2;

    /* 将认证套件初始化为0xff */
    memset_s(auth_policy, suites_num, 0xff, suites_num);

    /* 获取认证类型 */
    for (temp = 0; temp < suites_num; temp++) {
        /* AKM Suite List 长度4*n, 先判断长度在取值 */
        if (MAC_IE_REAMIN_LEN_IS_ENOUGH(2, *index, ie_len, 4) == OAL_FALSE) {
            OAM_ERROR_LOG0(0, OAM_SF_ANY, "{wal_check_auth_policy::length illegal!}");
            return OAL_FAIL;
        }

        if (oal_memcmp(oui, ie_data + *index, MAC_OUI_LEN) != 0) {
            OAM_ERROR_LOG0(0, OAM_SF_ANY, "{wal_check_auth_policy::auth OUI illegal!}");
            return OAL_FAIL;
        }
        auth_policy[temp] = ie_data[*index + MAC_OUI_LEN];
        *index += 4;
    }
    return OAL_SUCC;
}
...
oal_uint32 wal_parse_rsn_ie(const oal_uint8 *puc_ie, mac_beacon_param_stru *pst_beacon_param)
{
    ...
    if (ie_len < MAC_MIN_RSN_LEN) {
        OAM_WARNING_LOG1(0, OAM_SF_ANY, "{wal_parse_rsn_ie::invalid RSN IE len[%d]!}", ie_len);
        return OAL_FAIL;
    }
    ...
    *puc_grp_policy = puc_ie[uc_index + MAC_OUI_LEN];
    ...
    /* 获取成对密钥套件 */
    suites_num = OAL_MAKE_WORD16(puc_ie[uc_index], puc_ie[uc_index + 1]);
    if (suites_num > MAC_PAIRWISE_CIPHER_SUITES_NUM) {
    ...
    uc_index += 2;
    for (us_temp = 0; us_temp < suites_num; us_temp++) {
        if (MAC_IE_REAMIN_LEN_IS_ENOUGH(2, uc_index, ie_len, 4) == OAL_FALSE) {
            return OAL_FAIL;
        }
    ...
    /* 获取并检测认证类型 */
    if (wal_check_auth_policy(puc_ie, ie_len, puc_oui, &uc_index, puc_auth_policy) != OAL_SUCC) {
        return OAL_FAIL;
    }
    ...
    wal_check_rsn_remain_len(puc_ie, ie_len, &uc_index, pst_beacon_param);

    return OAL_SUCC;
}


oal_uint32 wal_parse_wpa_ie(const oal_uint8 *puc_ie, mac_beacon_param_stru *pst_beacon_param)
{
    oal_uint8                    uc_index;
    oal_uint16                   us_temp;
    oal_uint16                   us_ver;
                                 ...
                                 mac_beacon_param_stru *pst_beacon_param)
{
    ...
    if (WLAN_WITP_CAPABILITY_PRIVACY & us_capability_info) {
        ...
        puc_wpa_ie = mac_find_vendor_ie(MAC_WLAN_OUI_MICROSOFT,
            MAC_OUITYPE_WPA, pst_beacon_info->tail, pst_beacon_info->tail_len);
                            ...
                            oal_beacon_parameters *pst_beacon_info)
{
    ...
    /* 找到wmm ie，顺便判断下uapsd是否使能 */
        ...
        uc_uapsd = OAL_FALSE;
#endif
        if (memcpy_s(st_write_msg.auc_value, WAL_MSG_WRITE_MAX_LEN, &uc_uapsd, OAL_SIZEOF(oal_uint32)) != EOK) {
            OAM_ERROR_LOG0(0, OAM_SF_ANY, "wal_parse_wmm_ie::memcpy_s failed!");
            return OAL_FAIL;
        }
        ...
        if (OAL_UNLIKELY(ul_ret != OAL_SUCC)) {
            ul_ret = OAL_FAIL;
            OAM_WARNING_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_CFG, "{wal_parse_wmm_ie::uapsd switch set failed.}");
        }
    }
#endif
    /* wmm 开启/关闭 标记 */
...
OAL_STATIC oal_bool_enum wal_check_support_basic_rate_6M(const oal_uint8 *puc_supported_rates_ie,
                                                         oal_uint8        uc_supported_rates_num,
                                                         const oal_uint8 *puc_extended_supported_rates_ie,
...
/* 功能描述 : 对函数wal_parse_protocol_mode的拆分 */
static uint32_t wal_parse_protocol_2g_mode(oal_beacon_parameters *pst_beacon_info,
                                           wlan_protocol_enum_uint8 *pen_protocol)
{
    ...
    puc_supported_rates_ie =
        mac_find_ie(MAC_EID_RATES, pst_beacon_info->head + 24 + us_offset, pst_beacon_info->head_len - us_offset);
    ...
    rate_nums = uc_supported_rates_num + uc_extended_supported_rates_num;
    if (rate_nums < uc_supported_rates_num) {
    ...
    if (rate_nums == 12) {
        /* 根据基本速率区分为 11gmix1 还是 11gmix2, 如果基本速率集支持 6M , 则判断为 11gmix2 */
                                              ...
                                              wlan_protocol_enum_uint8 *pen_protocol)
{
    ...
    if (en_band == WLAN_BAND_2G) {
        return wal_parse_protocol_2g_mode(pst_beacon_info, pen_protocol);
    }
...
OAL_STATIC oal_uint32 wal_parse_ht_vht_ie(mac_vap_stru           *pst_mac_vap,
                                          ...
                                          mac_beacon_param_stru  *pst_beacon_param)
{
    oal_uint8              *puc_ht_ie = OAL_PTR_NULL;
    ...
    ul_ret = wal_parse_protocol_mode(pst_mac_vap->st_channel.en_band, pst_beacon_info,
        puc_ht_ie, puc_vht_ie, &pst_beacon_param->en_protocol);
    if (ul_ret != OAL_SUCC) {
...
OAL_STATIC oal_int32 wal_cfg80211_sched_scan_stop(oal_wiphy_stru          *pst_wiphy,
    ...
    (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 4, 35))) || (_PRE_OS_VERSION_LITEOS == _PRE_OS_VERSION)
OAL_STATIC oal_int32 wal_cfg80211_sched_scan_stop(oal_wiphy_stru          *pst_wiphy,
                                                  oal_net_device_stru     *pst_netdev);
#else
/* do nothing */
...
#endif
{
    ...
    if ((pst_wiphy == OAL_PTR_NULL) || (pst_request == OAL_PTR_NULL)) {
        OAM_ERROR_LOG0(0, OAM_SF_CFG, "{wal_cfg80211_scan::scan failed, null ptr}");
        goto fail;
    }
    ...
    if (pst_hmac_device == NULL) {
        OAM_ERROR_LOG1(0, OAM_SF_CFG, "{wal_cfg80211_scan::scan failed, null ptr, pst_hmac_device = null device_id:%d}",
...
fail:
    ...
    (_PRE_OS_VERSION_LITEOS == _PRE_OS_VERSION) || (_PRE_OS_VERSION_WIN32 == _PRE_OS_VERSION)
                                          ...
                                          oal_cfg80211_connect_params_stru *pst_sme)
{
    ...
    if ((pst_sme->key_len != 0) && (pst_sme->crypto.n_akm_suites == 0)) {
        ...
        uc_pairwise_cipher_num =
            OAL_MIN(pst_connect_param->st_crypto.n_ciphers_pairwise, OAL_NL80211_MAX_NR_CIPHER_SUITES);
        ...
        pst_connect_param->st_crypto.n_ciphers_pairwise += (oal_uint8)(puc_ie[ul_offset + 1] << 8); //lint !e679
        ...
        pst_connect_param->st_crypto.n_akm_suites += (oal_uint8)(puc_ie[ul_offset + 1] << 8); //lint !e679
                               ...
                               oal_cfg80211_connect_params_stru      *pst_sme)
{
    ...
    mac_vap_stru                       *pst_mac_vap = OAL_PTR_NULL;
    mac_device_stru                    *pst_mac_device = OAL_PTR_NULL;
    mac_vap_stru                       *pst_mac_vap_ap = OAL_PTR_NULL;

    if ((pst_wiphy == OAL_PTR_NULL) || (pst_net_device == OAL_PTR_NULL) || (pst_sme == OAL_PTR_NULL)) {
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "{wal_cfg80211_connect::ptr null}");
        return -OAL_EINVAL;
    }
#ifdef _PRE_WLAN_FEATURE_DFR
    if (g_st_dfr_info.bit_device_reset_process_flag) {
    ...
    pst_mac_vap = OAL_NET_DEV_PRIV(pst_net_device);
    if (pst_mac_vap == OAL_PTR_NULL) {
        OAM_WARNING_LOG0(0, OAM_SF_UM, "{wal_cfg80211_connect::pst_mac_vap null.}");
        return OAL_ERR_CODE_PTR_NULL;
    }

    pst_mac_device = mac_res_get_dev(pst_mac_vap->uc_device_id);
    if (pst_mac_device == OAL_PTR_NULL) {
        OAM_WARNING_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_UM, "{wal_cfg80211_connect::pst_mac_device null.}");
        return OAL_ERR_CODE_PTR_NULL;
    }

    /* 当前是STA模式时，判断是否为共存，共存时，若AP已关联8 user，直接return fail */
    ul_ret = mac_device_find_up_ap(pst_mac_device, &pst_mac_vap_ap);
    if ((ul_ret == OAL_SUCC) && (pst_mac_vap_ap != OAL_PTR_NULL) &&
        (pst_mac_vap_ap->us_user_nums == WLAN_ACTIVE_USER_MAX_NUM)) {
        OAM_WARNING_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_UM,
            "{wal_cfg80211_connect::AP STA coexist and ap has max users, sta assoc fobid.}");
...
#else
    /* 设置认证类型,增加类型判断 */
    if (pst_sme->auth_type == NL80211_AUTHTYPE_AUTOMATIC) {
        st_mac_cfg80211_connect_param.en_auth_type  = RDEF_NL80211_AUTHTYPE_AUTOMATIC;
    } else {
        st_mac_cfg80211_connect_param.en_auth_type  = pst_sme->auth_type;
    }
    st_mac_cfg80211_connect_param.en_mfp            = g_al_host_init_params[WLAN_CFG_INIT_PMF_ENABLE] ;

    OAM_WARNING_LOG4(0, OAM_SF_ANY,
        "{wal_cfg80211_connect::start a new connect, ssid_len[%d], auth_type[%d], privacy[%d],mfp[%d]}\r\n",
        pst_sme->ssid_len, pst_sme->auth_type, pst_sme->privacy, st_mac_cfg80211_connect_param.en_mfp);
#endif
                                  ...
                                  oal_uint16 us_reason_code)
{
    ...
    if ((pst_wiphy == OAL_PTR_NULL) || (pst_net_device == OAL_PTR_NULL)) {
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "{wal_cfg80211_disconnect::ptr is null}");
        return -OAL_EINVAL;
    }
    ...
    if (pst_mac_user == OAL_PTR_NULL) {
        OAM_WARNING_LOG1(0, OAM_SF_ANY,
            "{wal_cfg80211_disconnect::mac_res_get_mac_user failed, user idx[%d]!}", pst_mac_vap->uc_assoc_vap_id);
        return OAL_SUCC;
    }

    if (memcpy_s(st_mac_cfg_kick_user_param.auc_mac_addr, WLAN_MAC_ADDR_LEN,
        pst_mac_user->auc_user_mac_addr, WLAN_MAC_ADDR_LEN) != EOK) {
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "wal_cfg80211_disconnect::memcpy_s failed!");
        return -OAL_EINVAL;
    }
...
#if (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION)
static int32_t wal_cfg80211_add_key_filling(mac_addkey_param_stru *payload_param, uint8_t key_index,
    ...
    const
#endif
    oal_uint8 *mac_addr, oal_key_params_stru *params)
#elif (_PRE_OS_VERSION_LITEOS == _PRE_OS_VERSION)
static int32_t wal_cfg80211_add_key_filling(mac_addkey_param_stru *payload_param, uint8_t key_index, bool en_pairwise,
    const oal_uint8 *mac_addr, oal_key_params_stru *params)
#elif (_PRE_OS_VERSION_WIN32 == _PRE_OS_VERSION)
static int32_t wal_cfg80211_add_key_filling(mac_addkey_param_stru *payload_param, uint8_t key_index,
    oal_bool_enum en_pairwise, const oal_uint8 *mac_addr, oal_key_params_stru *params)
#endif
{
    int32_t ret = EOK;

    payload_param->uc_key_index = key_index;

    memset_s(payload_param->auc_mac_addr, WLAN_MAC_ADDR_LEN, 0, WLAN_MAC_ADDR_LEN);
    if (OAL_PTR_NULL != mac_addr) {
        /* 不能使用内核下发的mac指针，可能被释放，需要拷贝到本地再使用 */
        ret += memcpy_s(payload_param->auc_mac_addr, OAL_MAC_ADDR_LEN, mac_addr, WLAN_MAC_ADDR_LEN);
    }

#if (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION) || (_PRE_OS_VERSION_LITEOS == _PRE_OS_VERSION)
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,4,35) || (_PRE_OS_VERSION_LITEOS == _PRE_OS_VERSION)
    payload_param->en_pairwise  = en_pairwise;
#else
    payload_param->en_pairwise  = (OAL_PTR_NULL != mac_addr) ? OAL_TRUE : OAL_FALSE;
#endif
#else
    payload_param->en_pairwise  = en_pairwise;
#endif
    ...
    /* 2.2 获取相关密钥值 */
    payload_param->st_key.key_len = params->key_len;
    payload_param->st_key.seq_len = params->seq_len;
    payload_param->st_key.cipher  = params->cipher;
    ret += memcpy_s(payload_param->st_key.auc_key, OAL_WPA_KEY_LEN,
        params->key, (uint32_t)params->key_len);
    ret += memcpy_s(payload_param->st_key.auc_seq, OAL_WPA_SEQ_LEN,
        params->seq, (uint32_t)params->seq_len);
...
#endif
{
    ...
    /* 1.1 入参检查 */
    if ((OAL_PTR_NULL == pst_wiphy) || (OAL_PTR_NULL == pst_netdev) || (OAL_PTR_NULL == pst_params)) {
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "{wal_cfg80211_add_key::ptr null}");
        return -OAL_EINVAL;
    }
...
#if _PRE_OS_VERSION_LINUX == _PRE_OS_VERSION
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,4,35)
    l_ret += wal_cfg80211_add_key_filling(&st_payload_params, uc_key_index, en_pairwise, puc_mac_addr, pst_params);
#else
    l_ret += wal_cfg80211_add_key_filling(&st_payload_params, uc_key_index, puc_mac_addr, pst_params);
#endif
#endif
    ...
    /* 3.2 填写 msg 消息体 */
    l_ret += memcpy_s(st_write_msg.auc_value, WAL_MSG_WRITE_MAX_LEN,
        &st_payload_params, OAL_SIZEOF(mac_addkey_param_stru));
    if (l_ret != EOK) {
        OAM_WARNING_LOG1(0, OAM_SF_ANY, "wal_cfg80211_add_key::memcpy_s failed! l_ret[%d]", l_ret);
    }
...
#endif
{
    ...
    if (OAL_UNLIKELY(OAL_PTR_NULL == pst_wiphy) || OAL_UNLIKELY(OAL_PTR_NULL == pst_netdev) ||
        ...
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "{wal_cfg80211_get_key::ptr null}");
        return -OAL_EINVAL;
    }
    ...
    if (OAL_PTR_NULL != puc_mac_addr) {
        /* 不能使用内核下发的mac指针，可能被释放，需要拷贝到本地再使用 */
        if (memcpy_s(auc_mac_addr, WLAN_MAC_ADDR_LEN, puc_mac_addr, WLAN_MAC_ADDR_LEN) != EOK) {
            OAM_ERROR_LOG0(0, OAM_SF_ANY, "wal_cfg80211_get_key::memcpy_s failed!");
            return -OAL_EINVAL;
        }
    ...
    /* 3.2 填写 msg 消息体 */
    if (memcpy_s(st_write_msg.auc_value, WAL_MSG_WRITE_MAX_LEN, &st_payload_params,
                OAL_SIZEOF(mac_getkey_param_stru)) != EOK) {
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "wal_cfg80211_get_key::memcpy_s failed!");
        return -OAL_EINVAL;
    }
...
#if (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION) || (_PRE_OS_VERSION_LITEOS == _PRE_OS_VERSION)
oal_int32 wal_cfg80211_remove_key(oal_wiphy_stru *pst_wiphy, oal_net_device_stru *pst_netdev, oal_uint8 uc_key_index,
...
#endif
#else
OAL_STATIC oal_int32 wal_cfg80211_remove_key(oal_wiphy_stru *pst_wiphy, oal_net_device_stru *pst_netdev,
    oal_uint8 uc_key_index, oal_bool_enum en_pairwise, OAL_CONST oal_uint8 *puc_mac_addr)
#endif
{
    ...
    /* 1.1 入参检查 */
    if (OAL_UNLIKELY(OAL_PTR_NULL == pst_wiphy) || OAL_UNLIKELY(OAL_PTR_NULL == pst_netdev)) {
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "{wal_cfg80211_remove_key::Param Check ERROR}");
        return -OAL_EINVAL;
    }
    ...
    if (l_ret != EOK) {
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "wal_cfg80211_remove_key::memcpy_s failed!");
        return -OAL_EFAIL;
    }
    if (OAL_SUCC != wal_send_cfg_event(pst_netdev, WAL_MSG_TYPE_WRITE,
...
OAL_STATIC int32_t wal_cfg80211_send_common_msg(oal_net_device_stru *netdev, uint8_t *payload,
    uint16_t payload_len, wlan_cfgid_enum_uint16 cfgid)
{
    int32_t ret;
    wal_msg_write_stru write_msg = {0};

    /* 填写 msg 消息头 */
    write_msg.en_wid = cfgid;
    write_msg.us_len = payload_len;

    /* 填写 msg 消息体 */
    if (memcpy_s(write_msg.auc_value, WAL_MSG_WRITE_MAX_LEN, payload, payload_len) != EOK) {
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "wal_cfg80211_send_common_msg::memcpy_s failed!");
        return -OAL_EFAIL;
    }

    ret = wal_send_cfg_event(netdev,
                             WAL_MSG_TYPE_WRITE,
                             WAL_MSG_WRITE_MSG_HDR_LENGTH + payload_len,
                             (uint8_t *)&write_msg,
                             OAL_FALSE,
                             OAL_PTR_NULL);
    if (ret != OAL_SUCC) {
        OAM_WARNING_LOG1(0, OAM_SF_ANY, "{wal_cfg80211_send_common_msg::send_cfg_event return err code %d!}", ret);
        return -OAL_EFAIL;
    }
...
#endif
{
    ...
    if (OAL_UNLIKELY(pst_wiphy == OAL_PTR_NULL) || OAL_UNLIKELY(pst_netdev == OAL_PTR_NULL)) {
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "{wal_cfg80211_set_default_key::ptr is null}");
        return -OAL_EINVAL;
    }
    ...
    return wal_cfg80211_send_common_msg(pst_netdev, (uint8_t *)&st_payload_params,
                                        OAL_SIZEOF(mac_setdefaultkey_param_stru), WLAN_CFGID_DEFAULT_KEY);
}


int32_t wal_cfg80211_set_default_mgmt_key(oal_wiphy_stru *wiphy, oal_net_device_stru *netdev, uint8_t key_index)
{
    mac_setdefaultkey_param_stru payload_params = {0};

    /* 入参检查 */
    if (OAL_ANY_NULL_PTR2(wiphy, netdev)) {
        OAM_ERROR_LOG2(0, OAM_SF_ANY, "{wal_cfg80211_set_default_mgmt_key::wiphy or netdev ptr is null,error %x, %x!}",
                       (uintptr_t)wiphy, (uintptr_t)netdev);
        return -OAL_EINVAL;
    }

    /* 消息参数准备 */
    payload_params.uc_key_index = key_index;
    payload_params.en_unicast   = OAL_FALSE;
    payload_params.en_multicast = OAL_TRUE;

    OAM_WARNING_LOG1(0, OAM_SF_ANY, "{wal_cfg80211_set_default_mgmt_key::key_index:%d}", key_index);

    return wal_cfg80211_send_common_msg(netdev, (uint8_t *)&payload_params,
                                        OAL_SIZEOF(mac_setdefaultkey_param_stru), WLAN_CFGID_DEFAULT_KEY);
}
...
/*lint -e40*/
#if ((LINUX_VERSION_CODE <= KERNEL_VERSION(3, 4, 35)) && (_PRE_OS_VERSION_LITEOS != _PRE_OS_VERSION)) || \
    ((_PRE_OS_VERSION_WIN32_RAW == _PRE_OS_VERSION) ||(_PRE_OS_VERSION_WIN32 == _PRE_OS_VERSION))
OAL_STATIC oal_int32 wal_cfg80211_set_channel(oal_wiphy_stru           *pst_wiphy,
#if (LINUX_VERSION_CODE > KERNEL_VERSION(2, 6, 34)) && (_PRE_OS_VERSION_WIN32 != _PRE_OS_VERSION)
                                              oal_net_device_stru     *pst_netdev,
#endif
                                              oal_ieee80211_channel    *pst_chan,
                                              oal_nl80211_channel_type  en_channel_type)
{
    g_en_channel_type = en_channel_type;
    /* 通过HOSTAPD 设置信道采用接口wal_ioctl_set_channel */
    OAM_WARNING_LOG1(0, OAM_SF_ANY, "{wal_cfg80211_set_channel::g_en_channel_type = %d!}\r\n", g_en_channel_type);
    return OAL_SUCC;
}
#endif
/*lint +e40*/
                                             ...
                                             oal_beacon_parameters *pst_beacon_info)
{
    ...
    oal_uint8                uc_vap_id;
    oal_uint8               *puc_ie = OAL_PTR_NULL;
    mac_cfg_ssid_param_stru *pst_ssid_param = OAL_PTR_NULL;
    oal_uint8                uc_ssid_len;
    oal_uint32               l_memcpy_ret = EOK;

    if (OAL_ANY_NULL_PTR3(pst_wiphy, pst_wiphy, pst_beacon_info)) {
    ...
    if (pst_beacon_info->head == OAL_PTR_NULL) {
        OAM_ERROR_LOG1(0, OAM_SF_ANY, "{wal_cfg80211_set_beacon::pst_beacon_info->head %x!}", pst_beacon_info->head);
        return -OAL_EINVAL;
    }
    ...
    OAM_INFO_LOG3(uc_vap_id, OAM_SF_ANY, "{wal_cfg80211_set_beacon::crypto_mode=%d, group_crypt=%d, en_protocol=%d!}",
        st_beacon_param.uc_crypto_mode, st_beacon_param.uc_group_crypto, st_beacon_param.en_protocol);

    OAM_INFO_LOG3(uc_vap_id, OAM_SF_ANY, "auth_type[0]=%d, auth_type[1]=%d, hidden_ssid=%d",
        st_beacon_param.auc_auth_type[0], st_beacon_param.auc_auth_type[1], st_beacon_param.uc_hidden_ssid);
    ...
    l_memcpy_ret += memcpy_s(st_write_msg.auc_value,
        WAL_MSG_WRITE_MAX_LEN, &st_beacon_param, OAL_SIZEOF(mac_beacon_param_stru));
    ...
    if (uc_ssid_len != 0) {
        ...
        if (l_memcpy_ret != EOK) {
            OAM_ERROR_LOG0(0, OAM_SF_ANY, "wal_cfg80211_add_beacon::memcpy_s failed!");
            return -OAL_EFAIL;
        }
        l_ret = wal_send_cfg_event(pst_dev,
                                   WAL_MSG_TYPE_WRITE,
                                             ...
                                             oal_beacon_parameters *pst_beacon_info)
{
    ...
    if (pst_beacon_info->head == OAL_PTR_NULL) {
        OAM_ERROR_LOG1(0, OAM_SF_ANY, "{wal_cfg80211_add_beacon::pst_beacon_info->head %x!}", pst_beacon_info->head);
        return -OAL_EINVAL;
    }
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 4, 35)) || \
    (_PRE_OS_VERSION_WIN32 == _PRE_OS_VERSION) || (_PRE_OS_VERSION_LITEOS == _PRE_OS_VERSION)
                                           ...
                                           oal_uint8           uc_ssid_len)
{
    ...
    pst_ssid_param = (mac_cfg_ssid_param_stru *)(st_write_msg.auc_value);
    pst_ssid_param->uc_ssid_len = uc_ssid_len;
    if (memcpy_s(pst_ssid_param->ac_ssid, WLAN_SSID_MAX_LEN, (oal_int8 *)puc_ssid_ie, uc_ssid_len) != EOK) {
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "wal_cfg80211_set_ssid::memcpy_s failed!");
        return -OAL_EFAIL;
    }
                                                    ...
                                                    mac_beacon_param_stru *pst_beacon_param)
{
    ...
    oal_uint32  ul_loop;
    oal_uint32  ul_ret;
    oal_uint8  *puc_beacon_info_tmp = OAL_PTR_NULL;
    ...
    if (OAL_ANY_NULL_PTR3(pst_mac_vap, pst_beacon_info, pst_beacon_param)) {
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "{wal_cfg80211_fill_beacon_param::param is NULL}");
        return -OAL_EINVAL;
    }
    ...
    // 1.安全配置ie消息等
    if (pst_beacon_info->tail == OAL_PTR_NULL || pst_beacon_info->head == OAL_PTR_NULL) {
        OAM_ERROR_LOG0(uc_vap_id, OAM_SF_ANY, "{wal_cfg80211_fill_beacon_param::beacon frame error}");
        return -OAL_EINVAL;
    }

    /* 为避免读内存越界，传入beacon header长度不能小于帧头(24)+TSF(8)+BEACON_INTERVAL(2)+CAP_INFO(2) */
    if (pst_beacon_info->head_len <
        (MAC_80211_FRAME_LEN + MAC_TIME_STAMP_LEN + MAC_BEACON_INTERVAL_LEN + MAC_CAP_INFO_LEN)) {
    ...
    if (l_ret != EOK) {
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "wal_cfg80211_fill_beacon_param::memcpy_s failed!");
    ...
    OAM_WARNING_LOG3(uc_vap_id, OAM_SF_ANY,
        "{wal_cfg80211_fill_beacon_param::crypto_mode=%d, group_crypt=%d, en_protocol=%d!}",
                                     ...
                                     oal_beacon_data_stru   *pst_beacon_info)
{
    ...
    if ((pst_wiphy == OAL_PTR_NULL) || (pst_netdev == OAL_PTR_NULL) || (pst_beacon_info == OAL_PTR_NULL)) {
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "{wal_cfg80211_change_beacon::param null}");
        return -OAL_EINVAL;
    }
#ifdef _PRE_WLAN_FEATURE_DFR
    if (g_st_dfr_info.bit_device_reset_process_flag) {
    ...
    pst_mac_vap = OAL_NET_DEV_PRIV(pst_netdev);
    if (pst_mac_vap == OAL_PTR_NULL) {
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "{wal_cfg80211_change_beacon::pst_mac_vap null}");
        return -OAL_EINVAL;
    }
    ...
    /* 填写 msg 消息体 */
    if (memcpy_s(st_write_msg.auc_value, WAL_MSG_WRITE_MAX_LEN, &st_beacon_param,
        OAL_SIZEOF(mac_beacon_param_stru)) != EOK) {
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "wal_cfg80211_change_beacon::memcpy_s failed!");
        return -OAL_EFAIL;
    }
    ...
    l_ret = wal_send_cfg_event(pst_netdev,
                               ...
                               OAL_PTR_NULL);
    if (l_ret != OAL_SUCC) {
        OAM_WARNING_LOG1(0, OAM_SF_ANY, "{wal_cfg80211_change_beacon::wal_send_cfg_event fail ret[%d]}", l_ret);
        return -OAL_EFAIL;
    }
                                                   ...
                                                   oal_net_device_stru *pst_netdev)
{
    ...
    oal_ieee80211_channel               *pst_channel = OAL_PTR_NULL;
...
#else
    l_bandwidth    = g_en_channel_type;
    l_center_freq1 = pst_netdev->ieee80211_ptr->channel->center_freq;
    ...
    if (l_ret != OAL_SUCC) {
        OAM_WARNING_LOG2(uc_vap_id, OAM_SF_ANY,
            "{wal_cfg80211_set_channel::mac_is_channel_num_valid. band[%d],ch num[%d]!}", pst_channel->band, l_channel);
        return -OAL_EINVAL;
    }
    ...
    if (l_bandwidth_value == 80) {
        ...
        OAM_WARNING_LOG1(uc_vap_id, OAM_SF_ANY,
            "{wal_cfg80211_set_channel::FGPA is not support 80M,Set en_bandwidth = %d.", en_bandwidth);
#endif
    ...
    l_ret = wal_send_cfg_event(pst_netdev,
                               ...
                               &pst_rsp_msg);
    if (l_ret != OAL_SUCC) {
        OAM_WARNING_LOG1(0, OAM_SF_P2P, "{wal_cfg80211_set_channel_info::wal_send_cfg_event failed: [%d]!}\r\n", l_ret);
        return -OAL_EFAIL;
    }
...
/* 配置vap的auth类型 */
OAL_STATIC int32_t wal_cfg80211_set_auth_mode(oal_net_device_stru *netdev, uint8_t auth_algs)
{
#ifdef _PRE_WLAN_FEATURE_SAE
    wal_msg_write_stru st_write_msg = {0};
    int32_t            ret;
    ...
    if ((auth_algs != WLAN_WITP_AUTH_OPEN_SYSTEM) &&
        ...
        OAM_WARNING_LOG1(0, OAM_SF_ANY, "{wal_cfg80211_set_auth_mode::en_auth_algs error[%d].}", auth_algs);
        return -OAL_EFAIL;
    }

    WAL_WRITE_MSG_HDR_INIT(&st_write_msg, WLAN_CFGID_AUTH_MODE, OAL_SIZEOF(auth_algs));
    *((uint8_t *)(st_write_msg.auc_value)) = auth_algs;  /* 设置配置命令参数 */

    ret = wal_send_cfg_event(netdev,
                             WAL_MSG_TYPE_WRITE,
                             WAL_MSG_WRITE_MSG_HDR_LENGTH + OAL_SIZEOF(auth_algs),
...
static int32_t wal_cfg80211_start_ap_section1(oal_wiphy_stru *wiphy, oal_net_device_stru *netdev,
    oal_ap_settings_stru *ap_settings, uint8_t vap_id)
{
    int32_t ret;
    int32_t ssid_param_len;
    uint8_t ssid_len;
    uint8_t *puc_ssid_ie = OAL_PTR_NULL;
    uint8_t ssid_ie[IEEE80211_MAX_SSID_LEN];

    // 1.设置信道
    ret = wal_cfg80211_set_channel_info(wiphy, netdev);
    if (ret != OAL_SUCC) {
        OAM_WARNING_LOG1(vap_id, OAM_SF_ANY, "{wal_cfg80211_start_ap_section1::failed to set channel, code[%x]}", ret);
        return -OAL_EFAIL;
    }

    // 2.设置ssid等信息
    ssid_param_len = ap_settings->ssid_len;
    if ((ssid_param_len > IEEE80211_MAX_SSID_LEN) || (ssid_param_len <= 0)) {
        OAM_WARNING_LOG1(vap_id, OAM_SF_ANY, "wal_cfg80211_start_ap_section1::ssid len error, len[%d]", ssid_param_len);
        return -OAL_EFAIL;
    }

    memset_s(ssid_ie, sizeof(ssid_ie), 0, sizeof(ssid_ie));
    ret = memcpy_s(ssid_ie, sizeof(ssid_ie), ap_settings->ssid, (oal_uint32)ssid_param_len);
    if (ret != EOK) {
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "wal_cfg80211_start_ap_section1::memcpy_s failed!");
        return -OAL_EFAIL;
    }

    puc_ssid_ie = ssid_ie;
    ssid_len = (oal_uint8)ssid_param_len;
    if (ssid_len != 0) {
        ret = wal_cfg80211_set_ssid(netdev, puc_ssid_ie, ssid_len);
        if (ret != OAL_SUCC) {
            OAM_WARNING_LOG1(vap_id, OAM_SF_ANY,
                "{wal_cfg80211_start_ap_section1::fail to send ssid cfg msg, error[%d]}", ret);
            return -OAL_EFAIL;
        }
    }

    // 2.2 设置auth mode信息
    ret = wal_cfg80211_set_auth_mode(netdev, ap_settings->auth_type);
    if (ret != OAL_SUCC) {
        OAM_WARNING_LOG1(vap_id, OAM_SF_ANY, "{wal_cfg80211_start_ap_section1::set auth_tpye fail, error[%d]}", ret);
        return -OAL_EFAIL;
    }
...
oal_int32 wal_cfg80211_start_ap(oal_wiphy_stru *pst_wiphy, oal_net_device_stru *pst_netdev,
    oal_ap_settings_stru *pst_ap_settings)
{
    mac_beacon_param_stru        st_beacon_param;  /* beacon info struct */
    wal_msg_write_stru           st_write_msg;
    mac_vap_stru                *pst_mac_vap = OAL_PTR_NULL;
    oal_beacon_data_stru        *pst_beacon_info = OAL_PTR_NULL;
    oal_int32                    l_ret;
    oal_uint8                    uc_vap_id;
    oal_uint32                   l_memcpy_ret = EOK;
    ...
    if (OAL_ANY_NULL_PTR3(pst_wiphy, pst_netdev, pst_ap_settings)) {
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "{wal_cfg80211_start_ap::param null}");
        return -OAL_EINVAL;
    }
    ...
    pst_mac_vap = OAL_NET_DEV_PRIV(pst_netdev);
    if (pst_mac_vap == OAL_PTR_NULL) {
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "{wal_cfg80211_start_ap::pst_mac_vap = null}");
        return -OAL_EINVAL;
    }

    uc_vap_id = pst_mac_vap->uc_vap_id;

    l_ret = wal_cfg80211_start_ap_section1(pst_wiphy, pst_netdev, pst_ap_settings, uc_vap_id);
    if (l_ret != OAL_SUCC) {
        return l_ret;
    }
    ...
    if (l_memcpy_ret != EOK) {
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "wal_cfg80211_start_ap::memcpy_s failed!");
        return -OAL_EFAIL;
    }
    /* 发送消息 */
    ...
    if (l_ret != OAL_SUCC) {
        OAM_WARNING_LOG1(uc_vap_id, OAM_SF_ANY,
            "{wal_cfg80211_start_ap::failed to start addset beacon, error[%d]}", l_ret);
        return -OAL_EFAIL;
    }
...
oal_int32 wal_cfg80211_stop_ap(oal_wiphy_stru *pst_wiphy, oal_net_device_stru *pst_netdev)
{
    ...
    if ((pst_wiphy == OAL_PTR_NULL) || (pst_netdev == OAL_PTR_NULL)) {
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "{wal_cfg80211_stop_ap::param null}");
        return -OAL_EINVAL;
    }
...
/* HI1131C modify begin */
    ...
    OAM_WARNING_LOG4(0, OAM_SF_SCAN,
        ...
        pst_request->n_channels, pst_request->interval, pst_request->flags, pst_request->min_rssi_thold);
#else
#if (LINUX_VERSION_CODE > KERNEL_VERSION(3, 4, 35)) || (_PRE_OS_VERSION_LITEOS == _PRE_OS_VERSION)
    OAM_WARNING_LOG4(0, OAM_SF_SCAN,
        "wal_cfg80211_print_sched_scan_req_info::channels[%d],interval[%d]ms,flags[%d],rssi_thold[%d]",
        pst_request->n_channels, pst_request->interval, pst_request->flags, pst_request->rssi_thold);
#else
        OAM_WARNING_LOG2(0, OAM_SF_SCAN, "wal_cfg80211_print_sched_scan_req_info::channels[%d],interval[%d]ms",
...
/* HI1131C modify end */
    ...
    if (l_ret < EOK) {
        OAM_ERROR_LOG0(0, OAM_SF_SCAN, "wal_cfg80211_print_sched_scan_req_info::snprintf_s failed !");
        return;
    }
                                                   ...
                                                   oal_cfg80211_sched_scan_request_stru  *pst_request)
{
    ...
    hmac_scan_stru                 *pst_scan_mgmt = OAL_PTR_NULL;
    mac_vap_stru                   *pst_mac_vap = OAL_PTR_NULL;
    oal_cfg80211_ssid_stru         *pst_ssid_tmp = OAL_PTR_NULL;
    ...
    if ((pst_wiphy == OAL_PTR_NULL) || (pst_netdev == OAL_PTR_NULL) || (pst_request == OAL_PTR_NULL)) {
        OAM_ERROR_LOG0(0, OAM_SF_CFG, "{wal_cfg80211_sched_scan_start::input param pointer is null}");
        return -OAL_EINVAL;
    }
#ifdef _PRE_WLAN_FEATURE_DFR
    if (g_st_dfr_info.bit_device_reset_process_flag) {
    ...
    if (pst_request->n_match_sets <= 0) {
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_SCAN,
            "{wal_cfg80211_sched_scan_start::match_sets = %d!}", pst_request->n_match_sets);
        return -OAL_EINVAL;
    }

    /* 初始化pno扫描的结构体信息 */
    memset_s(&st_pno_scan_info, OAL_SIZEOF(mac_pno_scan_stru), 0, OAL_SIZEOF(mac_pno_scan_stru));

    /* 将内核下发的匹配的ssid集合复制到本地 */
    for (l_loop = 0; l_loop < OAL_MIN(pst_request->n_match_sets, MAX_PNO_SSID_COUNT); l_loop++) {
        pst_ssid_tmp = (oal_cfg80211_ssid_stru *)&(pst_request->match_sets[l_loop].ssid);
        if (pst_ssid_tmp->ssid_len >= WLAN_SSID_MAX_LEN) {
            OAM_WARNING_LOG1(0, OAM_SF_SCAN,
                "{wal_cfg80211_sched_scan_start:: wrong ssid_len[%d]!}", pst_ssid_tmp->ssid_len);
            continue;
        }
        if (memcpy_s(st_pno_scan_info.ac_match_ssid_set[l_loop], WLAN_SSID_MAX_LEN,
            pst_ssid_tmp->ssid, pst_ssid_tmp->ssid_len) != EOK) {
            OAM_ERROR_LOG0(0, OAM_SF_ANY, "wal_cfg80211_sched_scan_start::memcpy_s failed!");
            return -OAL_EINVAL;
        }
...
/* HI1131C modify end */
    ...
    if (ul_ret != OAL_SUCC) {
        OAM_WARNING_LOG1(0, OAM_SF_SCAN,
            "{wal_cfg80211_sched_scan_start::wal_cfg80211_start_sched_scan fail[%d]!}", ul_ret);
        return -OAL_EBUSY;
    }
...
#endif
{
    /* 参数合法性检查 */
    if ((pst_wiphy == OAL_PTR_NULL) || (pst_netdev == OAL_PTR_NULL)) {
        OAM_ERROR_LOG0(0, OAM_SF_CFG, "{wal_cfg80211_sched_scan_stop::ptr null}");
        return -OAL_EINVAL;
    }
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4,12,0)) && (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION)
...
#endif
{
    ...
    if ((pst_wiphy == OAL_PTR_NULL) || (pst_params == OAL_PTR_NULL)) {
        OAM_ERROR_LOG0(0, OAM_SF_CFG, "{wal_cfg80211_change_virtual_intf::pst_wiphy or pst_params ptr is null}");
        return -OAL_EINVAL;
    }
    ...
    if (pst_net_dev->ieee80211_ptr->iftype == en_type) {
        OAM_WARNING_LOG1(0, OAM_SF_CFG,
            "{wal_cfg80211_change_virtual_intf::same iftype[%d],do not need change !}\r\n", en_type);
        return OAL_SUCC;
    }
...
#endif
            pst_net_dev->ieee80211_ptr->iftype = en_type; // P2P_DEVICE提前创建，不需要通过wpa_supplicant 创建
...
OAL_STATIC oal_int32 wal_cfg80211_add_station(oal_wiphy_stru *pst_wiphy, oal_net_device_stru *pst_dev,
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 16, 0))
    const
#endif
    oal_uint8 *puc_mac,
...
#else
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 16, 0)) || (_PRE_OS_VERSION_LITEOS == _PRE_OS_VERSION)
    const
#endif
    oal_uint8 *puc_mac
#endif
)
{
    ...
    if (WLAN_VAP_MODE_BSS_AP != pst_mac_vap->en_vap_mode) {
        OAM_ERROR_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_ANY,
            "{wal_cfg80211_del_station::WLAN_VAP_MODE_BSS_AP != vap_mode[%d]!}\r\n", pst_mac_vap->en_vap_mode);
        return -OAL_EINVAL;
    }
    ...
    if (memcpy_s(st_kick_user_param.auc_mac_addr, WLAN_MAC_ADDR_LEN, puc_mac, OAL_MAC_ADDR_LEN) != EOK) {
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "wal_cfg80211_del_station::memcpy_s failed!");
        return -OAL_EINVAL;
    }
    ...
    if (OAL_SUCC != uint_ret) {
        ...
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_ANY,
            "{wal_cfg80211_del_station::wal_cfg80211_start_disconnect fail[%d]!}\r\n", uint_ret);
        OAM_INFO_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_ANY,
            "{wal_cfg80211_del_station::1 user is failed to be deleted!}\r\n");
        return -OAL_EINVAL;
    }

    OAM_INFO_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_ANY, "{wal_cfg80211_del_station::1 user is deleted!}");
    return OAL_SUCC;
}


OAL_STATIC oal_int32 wal_cfg80211_change_station(oal_wiphy_stru *pst_wiphy, oal_net_device_stru *pst_dev,
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 16, 0))
    const
#endif
                                                   ...
                                                   oal_station_info_stru  *pst_stats)
{
    ...
    if (pst_stats->txrate.flags & RATE_INFO_FLAGS_VHT_MCS) {
        ...
        pst_sta_info->txrate.flags = pst_stats->txrate.flags & 0xFC;
    } else {
...
oal_uint8 wal_cfg80211_get_station_filter(mac_vap_stru *pst_mac_vap, oal_uint8 *puc_mac)
{
    ...
    ul_runtime = (oal_uint32)OAL_TIME_GET_RUNTIME(pst_hmac_user->ul_rssi_last_timestamp,
        ul_current_time); //lint !e573 !e40
    if (ul_runtime < WAL_GET_STATION_THRESHOLD) {
        return OAL_FALSE;
    }
...
oal_int32 wal_cfg80211_judgement_timer(oal_station_info_stru *pst_sta_info,
                                       hmac_vap_stru *pst_hmac_vap, oal_long i_leftime)
{
    if ((pst_sta_info == OAL_PTR_NULL) || (pst_hmac_vap == OAL_PTR_NULL)) {
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "{wal_cfg80211_judgement_timer::param null}");
        return -OAL_EINVAL;
    }

    if (i_leftime == 0) {
        /* 超时还没有上报扫描结束 */
        OAM_WARNING_LOG1(0, OAM_SF_ANY, "{wal_cfg80211_judgement_timer::query info wait for %ld ms timeout!}",
                         ((QUERY_STATION_INFO_TIME * 1000) / OAL_TIME_HZ)); //lint !e40
        return -OAL_EINVAL;
    } else if (i_leftime < 0) {
        /* 定时器内部错误 */
        OAM_WARNING_LOG1(0, OAM_SF_ANY, "{wal_cfg80211_judgement_timer::query info wait for %ld ms error!}",
                         ((QUERY_STATION_INFO_TIME * 1000) / OAL_TIME_HZ)); //lint !e40
        return -OAL_EINVAL;
    } else {
        /* 正常结束  */
        ...
        pst_sta_info->chload = (int32_t)pst_hmac_vap->station_info_extend.chload;
        OAM_WARNING_LOG1(0, OAM_SF_ANY, "{wal_cfg80211_judgement_timer::chload = %d.}", pst_sta_info->chload);
#endif
        OAM_WARNING_LOG1(0, OAM_SF_CFG, "{wal_cfg80211_judgement_timer::rssi %d.}",
            (oal_uint8)pst_hmac_vap->station_info.signal);
        return OAL_SUCC;
    }
}
    ...
    oal_uint8 *puc_mac, oal_station_info_stru *pst_sta_info)
{
    ...
    if ((pst_dev == OAL_PTR_NULL) || (puc_mac == OAL_PTR_NULL) || (pst_sta_info == OAL_PTR_NULL)) {
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "{wal_cfg80211_get_station::param null}");
        return -OAL_EINVAL;
    }
    ...
    if (pst_hmac_vap == OAL_PTR_NULL) {
        OAM_ERROR_LOG1(0, OAM_SF_ANY,
            "{wal_cfg80211_get_station::mac_res_get_hmac_vap fail.vap_id[%u]}", pst_mac_vap->uc_vap_id);
        return -OAL_EINVAL;
    }
...
#ifdef CONFIG_HW_GET_EXT_SIG
        pst_sta_info->filled |= BIT(NL80211_STA_INFO_CNAHLOAD);
        pst_sta_info->chload = (oal_int32)pst_hmac_vap->station_info_extend.chload;
#endif
        return OAL_SUCC;
    }
    ...
    /* 3.2 填写 msg 消息体 */
    if (memcpy_s(st_write_msg.auc_value, WAL_MSG_WRITE_MAX_LEN,
        pst_query_station_info, OAL_SIZEOF(dmac_query_station_info_request_event)) != EOK) {
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "wal_cfg80211_get_station::memcpy_s failed!");
        return -OAL_EINVAL;
    }
    ...
    if (OAL_UNLIKELY(l_ret != OAL_SUCC)) {
        OAM_WARNING_LOG1(uc_vap_id, OAM_SF_ANY,
            "{wal_cfg80211_get_station::wal_send_cfg_event return err code %d!}", l_ret);
        return -OAL_EFAIL;
    }
    /*lint -e730*/ /* info, boolean argument to function */
    i_leftime = OAL_WAIT_EVENT_INTERRUPTIBLE_TIMEOUT(pst_hmac_vap->query_wait_q,
        (pst_hmac_vap->station_info_query_completed_flag == OAL_TRUE), QUERY_STATION_INFO_TIME);//lint !e40
    l_ret = wal_cfg80211_judgement_timer(pst_sta_info, pst_hmac_vap, i_leftime);
    if (l_ret != OAL_SUCC) {
        OAM_WARNING_LOG0(0, OAM_SF_ANY, "wal_cfg80211_judgement_timer:: failed!");
        return -OAL_EINVAL;
    }
    return OAL_SUCC;
}
...
OAL_STATIC void wal_cfg80211_add_p2p_interface_init(oal_net_device_stru *net_dev,
                                                    mac_device_stru *mac_device)
{
    ...
    if (OAL_UNLIKELY(mac_device->st_p2p_info.pst_primary_net_device == NULL)) {
        wal_set_random_mac_addr(primary_mac_addr);
    } else {
        if (OAL_UNLIKELY(OAL_NETDEVICE_MAC_ADDR(mac_device->st_p2p_info.pst_primary_net_device) == NULL)) {
            OAM_ERROR_LOG0(0, OAM_SF_ANY,
                           "{wal_cfg80211_add_p2p_interface_init() pst_primary_net_device; addr is null}");
            return;
        }
        if (memcpy_s(primary_mac_addr, WLAN_MAC_ADDR_LEN,
            OAL_NETDEVICE_MAC_ADDR(mac_device->st_p2p_info.pst_primary_net_device),
    ...
    net_dev->dev_addr[0] |= 0x02;  // wlan0 MAC[0] bit1 需要设置为0
    net_dev->dev_addr[4] ^= 0x80;
}
...
OAL_STATIC oal_wireless_dev_stru *wal_cfg80211_add_virtual_intf(oal_wiphy_stru      *pst_wiphy,
                                                                ...
                                                                oal_uint32          *pul_flags,
                                                                oal_vif_params_stru *pst_params)
#elif (LINUX_VERSION_CODE > KERNEL_VERSION(3, 4, 35))
OAL_STATIC oal_wireless_dev_stru *wal_cfg80211_add_virtual_intf(oal_wiphy_stru      *pst_wiphy,
                                                                const char          *puc_name,
                                                                enum nl80211_iftype  en_type,
                                                                oal_uint32          *pul_flags,
...
#endif
{
    ...
    hmac_device_stru           *pst_hmac_device = OAL_PTR_NULL;
#if (_PRE_PRODUCT_ID != _PRE_PRODUCT_ID_HI1131C_HOST)
    hmac_vap_stru              *pst_p2p0_hmac_vap = OAL_PTR_NULL;
    ...
    oal_uint32                  ul_ret;
    oal_int32                   l_ret;
    mac_cfg_add_vap_param_stru *pst_add_vap_param = OAL_PTR_NULL;
    ...
    if (OAL_ANY_NULL_PTR3(pst_wiphy, puc_name, pst_params)) {
        OAM_ERROR_LOG0(0, OAM_SF_CFG, "{wal_cfg80211_add_virtual_intf::param null}");
    ...
    if (pst_hmac_device == OAL_PTR_NULL) {
        OAM_WARNING_LOG0(0, OAM_SF_ANY, "{wal_cfg80211_add_virtual_intf::hmac_device is null!}\r\n");
        return ERR_PTR(-ENODEV);
    }
    ...
    if (hmac_get_p2p_status(pst_hmac_device->ul_p2p_intf_status, P2P_STATUS_IF_DELETING) == OAL_TRUE) {
        ...
        l_timeout = OAL_WAIT_EVENT_INTERRUPTIBLE_TIMEOUT(pst_hmac_device->st_netif_change_event,
            ...
            OAL_MSECS_TO_JIFFIES(WAL_MAX_WAIT_TIME)); //lint !e666
    ...
    if (pst_cfg_mac_vap == OAL_PTR_NULL) {
        OAM_WARNING_LOG1(0, OAM_SF_ANY,
            "{wal_cfg80211_add_virtual_intf::pst_cfg_mac_vap is null vap_id:%d!}", uc_cfg_vap_id);
        return ERR_PTR(-ENODEV);
    }
    pst_cfg_hmac_vap = (hmac_vap_stru *)mac_res_get_hmac_vap(uc_cfg_vap_id);
    if (pst_cfg_hmac_vap == OAL_PTR_NULL) {
        OAM_WARNING_LOG1(0, OAM_SF_ANY,
            "{wal_cfg80211_add_virtual_intf::pst_cfg_hmac_vap is null vap_id:%d!", uc_cfg_vap_id);
        return ERR_PTR(-ENODEV);
    }
    pst_cfg_net_dev = pst_cfg_hmac_vap->pst_net_device;
    if (memcpy_s(auc_name, OAL_IF_NAME_SIZE, puc_name,
        OAL_MIN(OAL_IF_NAME_SIZE, OAL_STRLEN(puc_name))) != EOK) { //lint !e666
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "wal_cfg80211_add_virtual_intf::memcpy_s failed!");
        return ERR_PTR(-EINVAL);
    }

    /* 此函数第一个入参代表私有长度，此处不涉及为0 */
#if defined(_PRE_WLAN_FEATURE_FLOWCTL)
    pst_net_dev = oal_net_alloc_netdev_mqs(0, auc_name, oal_ether_setup, WAL_NETDEV_SUBQUEUE_MAX_NUM, 1);
#elif defined(_PRE_WLAN_FEATURE_OFFLOAD_FLOWCTL)
    pst_net_dev = oal_net_alloc_netdev_mqs(0, (oal_int8 *)auc_name, oal_ether_setup, WLAN_NET_QUEUE_BUTT, 1);
#else
    pst_net_dev = oal_net_alloc_netdev(0, auc_name, oal_ether_setup);
#endif
    if (OAL_UNLIKELY(pst_net_dev == OAL_PTR_NULL)) {
        OAM_ERROR_LOG0(pst_cfg_mac_vap->uc_vap_id, OAM_SF_ANY, "{wal_cfg80211_add_virtual_intf::pst_net_dev null!}");
        return ERR_PTR(-ENOMEM);
    }

    pst_wdev = (oal_wireless_dev_stru *)OAL_MEM_ALLOC(OAL_MEM_POOL_ID_LOCAL,
        OAL_SIZEOF(oal_wireless_dev_stru), OAL_FALSE);
    if (OAL_UNLIKELY(pst_wdev == OAL_PTR_NULL)) {
        OAM_ERROR_LOG0(pst_cfg_mac_vap->uc_vap_id, OAM_SF_ANY, "{wal_cfg80211_add_virtual_intf::pst_wdev is null!}");
        /* 异常处理，释放内存 */
        oal_net_free_netdev(pst_net_dev);
        return ERR_PTR(-ENOMEM);
    }
    ...
    if ((en_p2p_mode == WLAN_LEGACY_VAP_MODE) && (en_vap_mode == WLAN_VAP_MODE_BSS_STA)) {
        ...
        if (pst_mac_device->st_p2p_info.pst_primary_net_device == OAL_PTR_NULL) {
            ...
            OAM_ERROR_LOG0(pst_cfg_mac_vap->uc_vap_id, OAM_SF_ANY,
                "{wal_cfg80211_add_virtual_intf::should not go here!}");
            OAL_MEM_FREE(pst_wdev, OAL_FALSE);
            oal_net_free_netdev(pst_net_dev);
            return ERR_PTR(-ENODEV);
        }
    ...
    l_ret = wal_send_cfg_event(pst_cfg_net_dev, WAL_MSG_TYPE_WRITE,
                               ...
                               &pst_rsp_msg);
    if (l_ret != OAL_SUCC) {
        OAM_WARNING_LOG1(0, OAM_SF_ANY, "{wal_cfg80211_add_virtual_intf::wal_send_cfg_event failed ret[%d]!}", l_ret);
        /*lint -e801*/
        goto ERR_STEP;
        /*lint +e801*/
    }
    ...
    oal_net_device_stru *net_dev)
{
    ...
    mac_wiphy_priv_stru *wal_wiphy_priv = OAL_PTR_NULL;
    wal_wiphy_priv = oal_wiphy_priv(wiphy);
    if (wal_wiphy_priv == OAL_PTR_NULL) {
        OAM_WARNING_LOG0(0, OAM_SF_CFG, "{wal_cfg80211_check_is_primary_netdev::pst_wiphy_priv is null!}");
        return OAL_FALSE;
    }
    mac_device = wal_wiphy_priv->pst_mac_device;
...
#endif
{
    ...
    if (OAL_UNLIKELY((OAL_PTR_NULL == pst_wiphy) || (OAL_PTR_NULL == pst_wdev))) {
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "{wal_cfg80211_del_virtual_intf::pst_wiphy or pst_wdev null ptr error}");
        return -OAL_EINVAL;
    }
    ...
    if (OAL_UNLIKELY((OAL_PTR_NULL == pst_wiphy) || (OAL_PTR_NULL == pst_netdev))) {
        OAM_ERROR_LOG2(0, OAM_SF_ANY, "{wal_cfg80211_del_virtual_intf::pst_wiphy or pst_netdev null ptr error %d, %d!}",
                       pst_wiphy, pst_netdev);
        return -OAL_EINVAL;
    }
    pst_net_dev = pst_netdev;
#endif
    ...
    en_p2p_mode = wal_wireless_iftype_to_mac_p2p_mode(pst_wdev->iftype);
#else
    en_p2p_mode = wal_wireless_iftype_to_mac_p2p_mode(pst_net_dev->ieee80211_ptr->iftype);
#endif
    if (WLAN_P2P_BUTT == en_p2p_mode) {
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "{wal_cfg80211_del_virtual_intf::wal_wireless_iftype_to_mac_p2p_mode failed!}");
        return -OAL_EINVAL;
    }
    ...
    OAM_WARNING_LOG2(0, OAM_SF_ANY, "{wal_cfg80211_del_virtual_intf::ul_p2p_intf_status %d, del result: %d}",
                                                    ...
                                                    oal_uint64 *pull_cookie)
{
    ...
    if (OAL_ANY_NULL_PTR4(pst_wiphy, pst_wdev, pst_chan, pull_cookie)) {
        OAM_ERROR_LOG0(0, OAM_SF_P2P, "{wal_cfg80211_remain_on_channel::ptr is null}");
        return -OAL_EINVAL;
    }
    ...
    pst_netdev = pst_wdev->netdev;
        ...
        /* 1.1 入参检查 */
    if (OAL_ANY_NULL_PTR4(pst_wiphy, pst_wdev, pst_chan, pull_cookie)) {
        OAM_ERROR_LOG4(0, OAM_SF_P2P, "{wal_cfg80211_remain_on_channel::ptr is null 2, %d, %d!}\r\n",
            pst_wiphy, pst_net_dev, pst_chan, pull_cookie);
        return -OAL_EINVAL;
    }

    pst_netdev = pst_net_dev;
#endif
    ...
    *pull_cookie = ++pst_mac_device->st_p2p_info.ull_last_roc_id;   /* cookie值上层调用需要判断是否是这次的发送导致的callback */
    ...
    // 抛事件到wal层处理
    /* 3.1 填写 msg 消息头 */
    WAL_WRITE_MSG_HDR_INIT(&st_write_msg,
        WLAN_CFGID_CFG80211_REMAIN_ON_CHANNEL, OAL_SIZEOF(mac_remain_on_channel_param_stru));

    /* 3.2 填写 msg 消息体 */
    if (memcpy_s(st_write_msg.auc_value, WAL_MSG_WRITE_MAX_LEN, &st_remain_on_channel,
        OAL_SIZEOF(mac_remain_on_channel_param_stru)) != EOK) {
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "wal_cfg80211_remain_on_channel::memcpy_s failed!");
        return -OAL_EFAIL;
    }
    ...
    l_ret = wal_send_cfg_event(pst_netdev,
                               ...
                               &pst_rsp_msg);
    if (l_ret != OAL_SUCC) {
        OAM_ERROR_LOG1(0, OAM_SF_P2P, "{wal_cfg80211_remain_on_channel::wal_send_cfg_event failed ret[%d]!}", l_ret);
        return -OAL_EFAIL;
    }
...
#else
    oal_cfg80211_ready_on_channel(pst_net_dev, *pull_cookie, pst_chan, ul_duration, GFP_KERNEL);
#endif
                                                           ...
                                                           oal_uint64 ull_cookie)
{
    ...
    if ((pst_wiphy == OAL_PTR_NULL) || (pst_wdev == OAL_PTR_NULL)) {
        OAM_ERROR_LOG0(0, OAM_SF_P2P, "{wal_cfg80211_cancel_remain_on_channel::ptr is null}");
        return -OAL_EINVAL;
    }
    ...
    /* 3.2 填写 msg 消息体 */
    if (memcpy_s(st_write_msg.auc_value, WAL_MSG_WRITE_MAX_LEN, &st_cancel_remain_on_channel,
                 OAL_SIZEOF(mac_remain_on_channel_param_stru)) != EOK) {
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "wal_cfg80211_cancel_remain_on_channel::memcpy_s failed!");
        return -OAL_EFAIL;
    }
    ...
    if (l_ret != OAL_SUCC) {
        OAM_WARNING_LOG1(0, OAM_SF_P2P,
            "{wal_cfg80211_cancel_remain_on_channel::wal_send_cfg_event return err code: [%d]!}\r\n", l_ret);
        return -OAL_EFAIL;
    }
    ...
    if (ul_err_code != OAL_SUCC) {
        OAM_ERROR_LOG1(0, OAM_SF_P2P,
            "{wal_cfg80211_cancel_remain_on_channel::wal_send_cfg_event return err code:[%u]!}\r\n", ul_err_code);
        return -OAL_EFAIL;
    }
                                    ...
                                    oal_uint32       ul_current_time)
{
    ...
    for (uc_loops = 0; uc_loops < WAL_COOKIE_ARRAY_SIZE; uc_loops++) {
        ...
        if (oal_time_is_before(pst_tmp_cookie->ul_record_time + OAL_MSECS_TO_JIFFIES(WAL_MGMT_TX_TIMEOUT_MSEC))) {
            /* cookie array 中保存的cookie 值超时，清空cookie array 中超时的cookie */
...
OAL_STATIC oal_uint32 wal_check_cookie_from_array(const oal_uint8 *puc_cookie_bitmap, oal_uint8 uc_cookie_idx)
{
                                     ...
                                     mac_mgmt_frame_stru *pst_mgmt_tx_param)
{
    ...
    /* 3.2 填写 msg 消息体 */
    if (memcpy_s(st_write_msg.auc_value, WAL_MSG_WRITE_MAX_LEN, pst_mgmt_tx_param,
        OAL_SIZEOF(mac_mgmt_frame_stru)) != EOK) {
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "wal_mgmt_do_tx::memcpy_s failed!");
        return OAL_FAIL;
    }
    ...
    l_ret = wal_send_cfg_event(pst_netdev,
                               ...
                               OAL_FALSE,
                               OAL_PTR_NULL);
    if (OAL_UNLIKELY(l_ret != OAL_SUCC)) { //lint !e730
    ...
    i_leftime = OAL_WAIT_EVENT_INTERRUPTIBLE_TIMEOUT(pst_mgmt_tx->st_wait_queue,
        pst_mgmt_tx->mgmt_tx_complete == OAL_TRUE,  (oal_uint32)OAL_MSECS_TO_JIFFIES(WAL_MGMT_TX_TIMEOUT_MSEC));
    if (i_leftime == 0) {
        /* 定时器超时 */
        ...
        return (oal_uint32)((pst_mgmt_tx->mgmt_tx_status != OAL_SUCC) ? OAL_FAIL : OAL_SUCC);
    }
}

OAL_STATIC int32_t wal_cfg80211_get_vapid_and_pullcookie(oal_net_device_stru *netdev, uint8_t *vap_id_ptr,
    uint64_t *pull_cookie)
{
    ...
    if (netdev == OAL_PTR_NULL) {
        OAM_ERROR_LOG0(0, OAM_SF_CFG, "{wal_cfg80211_get_vapid_and_pullcookie::pst_netdev ptr is null!}");
        return -OAL_EINVAL;
    }

    mac_vap = OAL_NET_DEV_PRIV(netdev);
    if (mac_vap == OAL_PTR_NULL) {
        OAM_ERROR_LOG0(0, OAM_SF_CFG, "{wal_cfg80211_get_vapid_and_pullcookie::can't get mac vap from netdevice!}");
        return -OAL_EINVAL;
    }

    mac_device = (mac_device_stru *)mac_res_get_dev(mac_vap->uc_device_id);
    if (mac_device == OAL_PTR_NULL) {
        OAM_ERROR_LOG0(0, OAM_SF_CFG, "{wal_cfg80211_get_vapid_and_pullcookie::pst_mac_device ptr is null!}");
        return -OAL_EINVAL;
    }

    *vap_id_ptr = mac_vap->uc_device_id;
...
OAL_STATIC int32_t wal_cfg80211_check_chan_param(oal_net_device_stru *netdev, const oal_ieee80211_mgmt *mgmt_data,
    oal_size_t mgmt_len, oal_ieee80211_channel *src_chan, oal_ieee80211_channel *des_chan)
{
#ifdef _PRE_WLAN_FEATURE_SAE
    if (src_chan == NULL) {
        mac_vap_stru      *mac_vap = NULL;
        ...
        mac_vap = OAL_NET_DEV_PRIV(netdev);
        if (!IS_STA(mac_vap)) {
            OAM_ERROR_LOG2(0, OAM_SF_CFG, "{wal_cfg80211_check_chan_param::ptr is null! vap_mode[%d], p2p_mode [%d]}",
                           mac_vap->en_vap_mode, mac_vap->en_p2p_mode);
            return -OAL_EINVAL;
        }

        /* SAE发送auth commit/comfirm 帧，下发的pst_channel入参是NULL, 需要根据目的MAC在扫描结果中查找信道信息 */
        OAM_WARNING_LOG0(0, OAM_SF_SAE, "{wal_cfg80211_check_chan_param::STA tx frame, find channel from scan result}");
        /* 判断传入的帧长度是否够长 */
        if (mgmt_len < MAC_80211_FRAME_LEN) {
            OAM_WARNING_LOG1(0, OAM_SF_SAE, "{wal_cfg80211_check_chan_param::frame length is too short.}", mgmt_len);
            return -OAL_EINVAL;
        }

        if (memcpy_s(mac_addr, WLAN_MAC_ADDR_LEN, mgmt_data->da, WLAN_MAC_ADDR_LEN) != EOK) {
            OAM_ERROR_LOG0(0, OAM_SF_CFG, "wal_cfg80211_check_chan_param::memcpy fail!");
            return -OAL_EINVAL;
        }
        bss_dscr = (mac_bss_dscr_stru *)hmac_scan_get_scanned_bss_by_bssid(mac_vap, mac_addr);
        if (bss_dscr == NULL) {
            OAM_WARNING_LOG3(0, OAM_SF_SAE, "{wal_cfg80211_check_chan_param::can't find [%02X:XX:XX:XX:%02X:%02X].}",
                mac_addr[0], mac_addr[4], mac_addr[5]);
            return -OAL_EINVAL;
        }
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4,7,0))
        des_chan->band = (enum nl80211_band)bss_dscr->st_channel.en_band;
#else
        des_chan->band = (enum ieee80211_band)bss_dscr->st_channel.en_band;
#endif
        des_chan->center_freq = oal_ieee80211_channel_to_frequency(bss_dscr->st_channel.uc_chan_number,
                                                                   (enum ieee80211_band)bss_dscr->st_channel.en_band);
        des_chan->hw_value = bss_dscr->st_channel.uc_chan_number;
    } else {
        memcpy_s(des_chan, OAL_SIZEOF(oal_ieee80211_channel), src_chan, OAL_SIZEOF(oal_ieee80211_channel));
    }
#else
    if (src_chan == NULL) {
        OAM_ERROR_LOG0(0, OAM_SF_CFG, "{wal_cfg80211_check_chan_param:: pst_chan ptr is null!}");
        return -OAL_EINVAL;
    } else {
        memcpy_s(des_chan, OAL_SIZEOF(oal_ieee80211_channel), src_chan, OAL_SIZEOF(oal_ieee80211_channel));
    }
...
/* HI1131C modify begin */
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 18, 0))
    oal_ieee80211_channel       *pst_chan = OAL_PTR_NULL;
    OAL_CONST oal_uint8         *puc_buf = OAL_PTR_NULL;
    oal_size_t                   ul_len;
...
#if (LINUX_VERSION_CODE > KERNEL_VERSION(3,4,35)) || (_PRE_OS_VERSION_LITEOS == _PRE_OS_VERSION)
/* HI1131C modify end */
    /* 1.1 入参检查,wpa3流程pst_chan可能为空,参数判断放在后续流程 */
    if (OAL_ANY_NULL_PTR4(pst_wiphy, pst_wdev, pull_cookie, puc_buf)) {
        OAM_ERROR_LOG0(0, OAM_SF_CFG, "{wal_cfg80211_mgmt_tx::param null}");
        return -OAL_EINVAL;
    }
    /* 通过net_device 找到对应的mac_device_stru 结构 */
    pst_netdev = pst_wdev->netdev;
...
/* HI1131C modify end */
    /* 1.1 入参检查 */
    if (OAL_ANY_NULL_PTR5(pst_wiphy, pst_net_dev, pst_chan, pull_cookie, puc_buf))
    {
        OAM_ERROR_LOG0(0, OAM_SF_CFG, "{wal_cfg80211_mgmt_tx::ptr is null}");
        return -OAL_EINVAL;
    }
    ...
    pst_netdev = pst_net_dev;
#endif
    if (wal_cfg80211_get_vapid_and_pullcookie(pst_netdev, &vap_id, (uint64_t *)pull_cookie) != OAL_SUCC) {
        OAM_ERROR_LOG0(0, OAM_SF_CFG, "{wal_cfg80211_mgmt_tx::wal_cfg80211_get_vapid_and_pullcookie fail!}");
        return -OAL_EINVAL;
    }

    pst_hmac_vap = (hmac_vap_stru *)mac_res_get_hmac_vap(vap_id);
    if (pst_hmac_vap == OAL_PTR_NULL) {
...
#else
        oal_cfg80211_mgmt_tx_status(pst_net_dev, *pull_cookie, puc_buf, ul_len, OAL_TRUE, GFP_KERNEL);
#endif
        return OAL_SUCC;
    }
    memset_s(&st_chan, OAL_SIZEOF(oal_ieee80211_channel), 0, OAL_SIZEOF(oal_ieee80211_channel));
    if (wal_cfg80211_check_chan_param(pst_netdev, pst_mgmt, ul_len, pst_chan, &st_chan) != OAL_SUCC) {
        OAM_ERROR_LOG0(0, OAM_SF_CFG, "{wal_cfg80211_mgmt_tx::wal_cfg80211_check_chan_param fail!}");
        return -OAL_EINVAL;
    }
    /* 2.1 消息参数准备 */
    ...
    if (ul_ret != OAL_SUCC) {
        OAM_WARNING_LOG1(vap_id, OAM_SF_ANY, "{wal_cfg80211_mgmt_tx::Failed to add cookies, ul_ret[%d]!}", ul_ret);
        return -OAL_EINVAL;
    } else {
    ...
    do {
        ...
        uc_retry++;
    } while ((ul_ret != OAL_SUCC) &&
        oal_time_before(OAL_TIME_JIFFY, ul_start_time_stamp + OAL_MSECS_TO_JIFFIES(2 * WAL_MGMT_TX_TIMEOUT_MSEC)));
...
#else
        oal_cfg80211_mgmt_tx_status(pst_net_dev, *pull_cookie, puc_buf, ul_len, OAL_FALSE, GFP_KERNEL);
#endif
    } else {
        /* 正常结束  */
                               ...
                               oal_uint64 *pull_cookie)
{
    ...
    mac_device_stru                 *pst_mac_device = OAL_PTR_NULL;
    mac_vap_stru                    *pst_mac_vap = OAL_PTR_NULL;
    OAL_CONST oal_ieee80211_mgmt    *pst_mgmt = OAL_PTR_NULL;
    oal_int32                       l_ret = 0;
    oal_uint32                      ul_ret = 0;
    mac_mgmt_frame_stru             st_mgmt_tx;
    oal_long                         i_leftime;
    ...
    if ((OAL_PTR_NULL == pst_wiphy) || (OAL_PTR_NULL == pst_wdev) ||
        (OAL_PTR_NULL == pst_chan) || (OAL_PTR_NULL == pull_cookie) || (OAL_PTR_NULL == puc_buf)) {
        OAM_ERROR_LOG4(0, OAM_SF_CFG, "{wal_cfg80211_mgmt_tx::ptr null,error %d, %d!}",
    ...
    *pull_cookie = pst_p2p_info->ull_send_action_id++;   /* cookie值上层调用需要判断是否是这次的发送导致的callback */
    ...
    /* 3.2 填写 msg 消息体 */
    if (memcpy_s(st_write_msg.auc_value, WAL_MSG_WRITE_MAX_LEN, &st_mgmt_tx,
                OAL_SIZEOF(st_mgmt_tx)) != EOK) {
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "wal_cfg80211_mgmt_tx::memcpy_s failed!");
        return -OAL_EINVAL;
    }
    ...
    /*lint -e730*/
    i_leftime = OAL_WAIT_EVENT_INTERRUPTIBLE_TIMEOUT(pst_mgmt_tx->st_wait_queue,
        OAL_TRUE== pst_mgmt_tx->mgmt_tx_complete,  (oal_uint32)OAL_MSECS_TO_JIFFIES(WAL_MGMT_TX_TIMEOUT_MSEC));
   /*lint +e730*/
                                             ...
                                             oal_int32 timeout)
{
    ...
    if (OAL_UNLIKELY((pst_wiphy == OAL_PTR_NULL) || (pst_netdev == OAL_PTR_NULL))) {
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "{wal_cfg80211_set_power_mgmt::pst_wiphy or pst_wdev null ptr error}");
        return -OAL_EINVAL;
    }
...
{
    wal_msg_write_stru                  st_write_msg;
    mac_cfg80211_ft_ies_stru           *pst_mac_ft_ies = OAL_PTR_NULL;
    wal_msg_stru                       *pst_rsp_msg = OAL_PTR_NULL;
    oal_int32                           l_ret;
    uint32_t                            err_code;
    ...
    if (memcpy_s(pst_mac_ft_ies->auc_ie, MAC_MAX_FTE_LEN, pst_fties->ie, pst_fties->ie_len) != EOK) {
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "wal_cfg80211_update_ft_ies::memcpy_s failed!");
        return -OAL_EINVAL;
    }
    ...
    l_ret = wal_send_cfg_event(pst_netdev,
                               ...
                               &pst_rsp_msg);
    if (OAL_UNLIKELY(l_ret != OAL_SUCC)) {
        OAM_ERROR_LOG1(0, OAM_SF_ANY, "{wal_cfg80211_update_ft_ies::wal_send_cfg_event: return err code %d!}", l_ret);
        return -OAL_EFAIL;
    }
    ...
    if (err_code != OAL_SUCC) {
        OAM_WARNING_LOG1(0, OAM_SF_ANY,
            "{wal_cfg80211_update_ft_ies::wal_check_and_release_msg_resp err code:[%u]!}", err_code);
        return -OAL_EFAIL;
    }
...
#ifdef _PRE_WLAN_FEATURE_SAE
#if (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION)
/* 删除一个pmk缓存,只在漫游场景下使用pmk缓存, 1131K不支持漫游，WPA3会下发实际未使用，直接返回成功 */
OAL_STATIC int32_t wal_cfg80211_set_pmksa(oal_wiphy_stru          *wiphy,
...
/* 删除一个pmk缓存,只在漫游场景下使用pmk缓存, 1131K不支持漫游，WPA3会下发实际未使用，直接返回成功 */
OAL_STATIC int32_t wal_cfg80211_del_pmksa(oal_wiphy_stru          *wiphy,
                                          oal_net_device_stru     *net_device,
                                          oal_cfg80211_pmksa_stru *pmksa)
{
    return OAL_SUCC;
...
/* 清除所有pmk缓存 */
OAL_STATIC int32_t wal_cfg80211_flush_pmksa(oal_wiphy_stru *wiphy, oal_net_device_stru *net_device)
{
    return OAL_SUCC;
}
#endif
...
/* 执行内核下发 ext_auth 命令 */
OAL_STATIC int32_t wal_cfg80211_external_auth(oal_wiphy_stru *wiphy, oal_net_device_stru *netdev,
    struct cfg80211_external_auth_params *external_auth_params)
{
    uint32_t                    ret;
    int32_t                     mem_ret;
    hmac_external_auth_req_stru ext_auth;

    memset_s(&ext_auth, OAL_SIZEOF(ext_auth), 0, OAL_SIZEOF(ext_auth));
    ext_auth.status = external_auth_params->status;
    mem_ret = memcpy_s(ext_auth.bssid, WLAN_MAC_ADDR_LEN, external_auth_params->bssid, WLAN_MAC_ADDR_LEN);
    ext_auth.ssid.uc_ssid_len = OAL_MIN(external_auth_params->ssid.ssid_len, OAL_IEEE80211_MAX_SSID_LEN);
    mem_ret += memcpy_s(ext_auth.ssid.auc_ssid, OAL_IEEE80211_MAX_SSID_LEN,
                        external_auth_params->ssid.ssid, ext_auth.ssid.uc_ssid_len);
    if (mem_ret != EOK) {
    ...
    ret = wal_cfg80211_do_external_auth(netdev, &ext_auth);
    if (ret != OAL_SUCC) {
...
#endif
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3,4,35)) || (_PRE_OS_VERSION_LITEOS == _PRE_OS_VERSION)
    .mgmt_tx_cancel_wait      = wal_cfg80211_mgmt_tx_cancel_wait, //lint !e64
#endif
#if (LINUX_VERSION_CODE > KERNEL_VERSION(3,4,35)) || (_PRE_OS_VERSION_LITEOS == _PRE_OS_VERSION)
...
oal_uint32 wal_cfg80211_init(oal_void)
{
    ...
    for (ul_chip = 0; (ul_chip < ul_chip_max_num) && (ul_chip < WLAN_CHIP_MAX_NUM_PER_BOARD); ul_chip++) {
        for (uc_device = 0; (uc_device < pst_hmac_board->ast_chip[ul_chip].uc_device_nums) &&
            (uc_device < WLAN_DEVICE_MAX_NUM_PER_CHIP); uc_device++) {
            /* 获取device_id */
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3,14,0)) && (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION)
            wal_cfgvendor_init(pst_wiphy);
#endif
#endif
            PS_PRINT_WARNING("wiphy_register start.\n");
...
#if (_PRE_PRODUCT_ID == _PRE_PRODUCT_ID_HI1131C_HOST)
            PS_PRINT_WARNING("wal_init_wlan_netdev wlan0.\n");
            ...
            PS_PRINT_WARNING("wal_init_wlan_netdev p2p0.\n");
