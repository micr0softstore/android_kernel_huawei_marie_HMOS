...
{
    uint16_t        msg_offset;
    uint8_t         param_set_cnt, ac_loop, edca_param_set;
    mac_device_stru *mac_device = (mac_device_stru *)mac_res_get_dev(hmac_sta->st_vap_base_info.uc_device_id);
    ...
    if (ie_data != OAL_PTR_NULL) {
        /* ie_data[1]的长度是不包括自己和之前字节的长度总长 */
        if (ie_data[1] < (HMAC_WMM_QOS_PARAMS_HDR_LEN + WLAN_WME_AC_BUTT * HMAC_WMM_AC_PARAMS_RECORD_LEN)) {
            OAM_ERROR_LOG1(0, OAM_SF_ASSOC, "{hmac_sta_up_get_edca_params::ie len error[%d]!}", ie_data[1]);
            return;
        }
        /* 解析wmm ie是否携带EDCA参数 */
    ...
    if (mac_find_ie(MAC_EID_HT_CAP, payload, msg_len) != OAL_PTR_NULL) {
...
oal_uint32 hmac_sta_wait_asoc_rx(hmac_vap_stru *pst_hmac_sta, oal_void *pst_msg)
{
    ...
    if (en_asoc_status != MAC_SUCCESSFUL_STATUSCODE) {
        ...
        /* STA保存AP在assoc rsp帧中的status code */
            ...
            switch (puc_data[MAC_ACTION_OFFSET_ACTION]) {
                case MAC_BA_ACTION_ADDBA_REQ:
                    ...
                    hmac_btcoex_check_rx_same_baw_start_from_addba_req(pst_hmac_vap, pst_hmac_user,
                                                                       pst_frame_hdr, puc_data, frame_body_len);
#endif
                    break;
