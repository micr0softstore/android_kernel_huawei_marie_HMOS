...
oal_uint8 g_auc_avail_protocol_mode[WLAN_PROTOCOL_BUTT][WLAN_PROTOCOL_BUTT] = {
    ...
    { WLAN_LEGACY_11A_MODE, WLAN_LEGACY_11B_MODE, WLAN_LEGACY_11G_MODE, WLAN_MIXED_ONE_11G_MODE,
      WLAN_MIXED_ONE_11G_MODE, WLAN_HT_MODE, WLAN_HT_MODE, WLAN_HT_ONLY_MODE, WLAN_PROTOCOL_BUTT, WLAN_HT_11G_MODE },
    { WLAN_LEGACY_11A_MODE, WLAN_LEGACY_11B_MODE, WLAN_LEGACY_11G_MODE, WLAN_MIXED_ONE_11G_MODE,
      WLAN_MIXED_ONE_11G_MODE, WLAN_HT_MODE, WLAN_VHT_MODE, WLAN_HT_ONLY_MODE, WLAN_VHT_ONLY_MODE, WLAN_PROTOCOL_BUTT },
...
oal_void hmac_rx_ba_session_decr(hmac_vap_stru *pst_hmac_vap, oal_uint8 uc_tidno)
{
    ...
    OAM_WARNING_LOG2(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_BA,
        "{hmac_rx_ba_session_decr::tid[%d] rx_session[%d] remove.}", uc_tidno, pst_hmac_vap->uc_rx_ba_session_num);
}
...
oal_void hmac_tx_ba_session_decr(hmac_vap_stru *pst_hmac_vap, oal_uint8 uc_tidno)
{
    ...
    OAM_WARNING_LOG2(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_BA,
        "{hmac_tx_ba_session_decr::tid[%d] tx_session[%d] remove.}", uc_tidno, pst_hmac_vap->uc_tx_ba_session_num);
}
...
OAL_STATIC OAL_INLINE oal_void hmac_tx_ba_session_incr(hmac_vap_stru *pst_hmac_vap, oal_uint8 uc_tidno)
{
    ...
    OAM_WARNING_LOG2(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_BA,
        "{hmac_tx_ba_session_incr::tid[%d] tx_session[%d] setup.}", uc_tidno, pst_hmac_vap->uc_tx_ba_session_num);
}
#else
...
oal_void hmac_rx_ba_session_decr(mac_device_stru *pst_mac_device, oal_uint8 uc_tidno)
{
    ...
    OAM_WARNING_LOG2(0, OAM_SF_BA, "{hmac_rx_ba_session_decr::tid[%d] rx_session[%d] remove.}",
...
oal_void hmac_tx_ba_session_decr(mac_device_stru *pst_mac_device, oal_uint8 uc_tidno)
{
    ...
    OAM_WARNING_LOG2(0, OAM_SF_BA, "{hmac_tx_ba_session_decr::tid[%d] tx_session[%d] remove.}",
                                     ...
                                     oal_uint8       uc_tid)
{
    ...
    if (OAL_ANY_NULL_PTR3(pst_vap, puc_data, pst_tx_ba)) {
        OAM_ERROR_LOG0(0, OAM_SF_BA, "{hmac_mgmt_encap_addba_req::null param");
                                     ...
                                     oal_uint8       uc_status)
{
    ...
    if ((pst_vap->st_vap_base_info.en_vap_mode == WLAN_VAP_MODE_BSS_STA) &&
        (pst_vap->st_vap_base_info.en_p2p_mode == WLAN_LEGACY_VAP_MODE)) {
        if (pst_vap->hmac_vap_btcoex.ba_size > 0) {
            us_ba_param |= (oal_uint16)(pst_vap->hmac_vap_btcoex.ba_size << 6);   /* BIT6 */
        } else {
                                 ...
                                 oal_uint8                      reason)
{
    ...
    if (OAL_ANY_NULL_PTR3(pst_vap, puc_data, puc_addr)) {
        OAM_ERROR_LOG0(0, OAM_SF_BA, "{hmac_mgmt_encap_delba::null param}");
        return OAL_ERR_CODE_PTR_NULL;
    }
                                  ...
                                  mac_action_mgmt_args_stru *pst_action_args)
{
    ...
    if (OAL_ANY_NULL_PTR3(pst_hmac_vap, pst_hmac_user, pst_action_args)) {
        OAM_ERROR_LOG0(0, OAM_SF_BA, "{hmac_mgmt_tx_addba_req::null param}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    if (memcpy_s((oal_uint8 *)(oal_netbuf_data(pst_addba_req) + us_frame_len),
    ...
    oal_spin_lock_bh(&(pst_hmac_user->ast_tid_info[uc_tidno].st_ba_tx_info.ba_status_lock));
    /* 更新对应的TID信息 */
...
#endif
    oal_spin_unlock_bh(&(pst_hmac_user->ast_tid_info[uc_tidno].st_ba_tx_info.ba_status_lock));
    /* 启动ADDBA超时计时器 */
                                  ...
                                  oal_uint8       uc_status)
{
    ...
    if (OAL_ANY_NULL_PTR3(pst_hmac_vap, pst_hmac_user, pst_ba_rx_info)) {
        OAM_ERROR_LOG0(0, OAM_SF_BA, "{hmac_mgmt_tx_addba_rsp::send addba rsp failed, null param}");
        return OAL_ERR_CODE_PTR_NULL;
    }
                              ...
                              mac_action_mgmt_args_stru *pst_action_args)
{
...
#endif
    if (OAL_ANY_NULL_PTR3(pst_hmac_vap, pst_hmac_user, pst_action_args)) {
        OAM_ERROR_LOG0(0, OAM_SF_BA, "{hmac_mgmt_tx_delba::null param}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    if (pst_hmac_user->ast_tid_info[pst_action_args->ul_arg1].st_ba_tx_info.en_ba_status == DMAC_BA_INIT) {
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_BA, "{hmac_mgmt_tx_delba::en_ba_status=[%d].}",
            pst_hmac_user->ast_tid_info[pst_action_args->ul_arg1].st_ba_tx_info.en_ba_status);
    }
    ...
    us_frame_len = hmac_mgmt_encap_delba(pst_hmac_vap, (oal_uint8 *)OAL_NETBUF_HEADER(pst_delba),
        pst_action_args->puc_arg5, (oal_uint8)pst_action_args->ul_arg1, en_initiator,
        (oal_uint8)pst_action_args->ul_arg3);
    memset_s((oal_uint8*)&st_wlan_ctx_action, OAL_SIZEOF(st_wlan_ctx_action), 0, OAL_SIZEOF(st_wlan_ctx_action));
    st_wlan_ctx_action.us_frame_len       = us_frame_len;
    st_wlan_ctx_action.uc_hdr_len         = MAC_80211_FRAME_LEN;
    st_wlan_ctx_action.en_action_category = MAC_ACTION_CATEGORY_BA;
    ...
    if (memcpy_s((oal_uint8 *)(oal_netbuf_data(pst_delba) + us_frame_len),
                 (WLAN_MEM_NETBUF_SIZE2 - us_frame_len),
                 (oal_uint8 *)&st_wlan_ctx_action,
                 OAL_SIZEOF(dmac_ctx_action_event_stru)) != EOK) {
...
#ifdef _PRE_WLAN_1131_CHR
    hmac_chr_set_del_ba_info((oal_uint8)pst_action_args->ul_arg1, (uint16_t)pst_action_args->ul_arg3);
#endif
    ...
    ul_ret = frw_event_dispatch_event(pst_event_mem);
    if (ul_ret != OAL_SUCC) {
        OAM_ERROR_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_BA, "{hmac_mgmt_tx_delba::frw_event_dispatch_event failed[%d].}",
            ul_ret);
        oal_netbuf_free(pst_delba);
        FRW_EVENT_FREE(pst_event_mem);
    ...
    if (en_initiator == MAC_RECIPIENT_DELBA) {
        /* 更新对应的TID信息 */
        hmac_ba_reset_rx_handle(pst_device, &pst_hmac_user->ast_tid_info[pst_action_args->ul_arg1].pst_ba_rx_info,
            (oal_uint8)pst_action_args->ul_arg1, OAL_FALSE);
    } else {
        oal_spin_lock_bh(&(pst_hmac_user->ast_tid_info[pst_action_args->ul_arg1].st_ba_tx_info.ba_status_lock));
        /* 更新对应的TID信息 */
...
#endif
        oal_spin_unlock_bh(&(pst_hmac_user->ast_tid_info[pst_action_args->ul_arg1].st_ba_tx_info.ba_status_lock));
...
oal_uint32 hmac_mgmt_rx_addba_req(hmac_vap_stru *pst_hmac_vap, hmac_user_stru *pst_hmac_user,
                                  oal_uint8 *puc_payload, oal_uint32 frame_body_len)
{
    ...
    if ((!(pst_mac_vap->en_protocol >= WLAN_HT_MODE)) ||
        ...
        OAM_WARNING_LOG2(pst_mac_vap->uc_vap_id, OAM_SF_BA,
            "{hmac_mgmt_rx_addba_req::ampdu is not supprot or not open, vap protocol mode = %d, \
            user protocol mode = %d.}", pst_mac_vap->en_protocol, pst_hmac_user->st_user_base_info.en_protocol_mode);
        return OAL_SUCC;
    }
    ...
    FRW_TIMER_CREATE_TIMER(&(pst_hmac_user->ast_tid_info[uc_tid].st_ba_timer), hmac_ba_timeout_fn,
        pst_hmac_vap->us_rx_timeout[WLAN_WME_TID_TO_AC(uc_tid)], &(pst_ba_rx_stru->st_alarm_data),
    ...
    OAM_WARNING_LOG4(pst_mac_vap->uc_vap_id, OAM_SF_BA,
        "{hmac_mgmt_rx_addba_req::process addba req receive and send addba rsp, uc_tid[%d], uc_status[%d], \
        baw_start[%d], baw_size[%d]}\r\n",
...
oal_uint32 hmac_mgmt_rx_addba_rsp(hmac_vap_stru *pst_hmac_vap, hmac_user_stru *pst_hmac_user,
                                  oal_uint8 *puc_payload, oal_uint32 frame_body_len)
{
    ...
    if (OAL_ANY_NULL_PTR3(pst_hmac_vap, pst_hmac_user, puc_payload)) {
        OAM_ERROR_LOG0(0, OAM_SF_BA, "{hmac_mgmt_rx_addba_rsp::null param}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    if ((pst_tid->st_ba_tx_info.en_ba_status == DMAC_BA_COMPLETE) ||
        ...
        OAM_WARNING_LOG4(pst_mac_vap->uc_vap_id, OAM_SF_BA,
            "{hmac_mgmt_rx_addba_rsp::addba rsp is received whne ba status is DMAC_BA_COMPLETE or uc_dialog_token \
            wrong.tid[%d], status[%d], rsp dialog[%d], req dialog[%d]}",
    ...
    /* 填写事件头 */
    FRW_EVENT_HDR_INIT(&(pst_hmac_to_dmac_crx_sync->st_event_hdr), FRW_EVENT_TYPE_WLAN_CTX,
                       DMAC_WLAN_CTX_EVENT_SUB_TYPE_BA_SYNC, OAL_SIZEOF(dmac_ctx_action_event_stru),
    ...
    if (pst_rx_addba_rsp_event->uc_stauts != MAC_SUCCESSFUL_STATUSCODE) {
        oal_spin_lock_bh(&(pst_tid->st_ba_tx_info.ba_status_lock));
        /* 重置HMAC模块信息 */
...
#endif
        oal_spin_unlock_bh(&(pst_tid->st_ba_tx_info.ba_status_lock));
    } else {
...
oal_uint32 hmac_mgmt_rx_delba(hmac_vap_stru *pst_hmac_vap, hmac_user_stru *pst_hmac_user,
                              oal_uint8 *puc_payload, oal_uint32 frame_body_len)
{
    frw_event_mem_stru         *pst_event_mem = OAL_PTR_NULL;      /* 申请事件返回的内存指针 */
    frw_event_stru             *pst_hmac_to_dmac_crx_sync = OAL_PTR_NULL;
    ...
    if (OAL_UNLIKELY((pst_hmac_vap == OAL_PTR_NULL) || (pst_hmac_user == OAL_PTR_NULL) ||
        ...
        OAM_ERROR_LOG0(0, OAM_SF_BA, "{hmac_mgmt_rx_delba::null param}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    if (frame_body_len < MAC_ADDBA_REQ_FRAME_BODY_LEN) {
        OAM_WARNING_LOG1(0, OAM_SF_BA, "{frame_body_len[%d] < MAC_ADDBA_REQ_FRAME_BODY_LEN.}", frame_body_len);
    ...
    OAM_WARNING_LOG3(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_BA,
        "{hmac_mgmt_rx_delba::receive delba from peer sta, tid[%d], uc_initiator[%d], reason[%d].}", uc_tid,
        uc_initiator, us_reason);
                                    ...
                                    mac_priv_req_args_stru *pst_priv_req)
{
    ...
    if (OAL_ANY_NULL_PTR3(pst_hmac_vap, pst_hmac_user, pst_priv_req)) {
        OAM_ERROR_LOG0(0, OAM_SF_AMPDU, "{hmac_mgmt_tx_ampdu_start::param null}");
        return OAL_ERR_CODE_PTR_NULL;
    }
                                  ...
                                  mac_priv_req_args_stru *pst_priv_req)
{
    ...
    if (OAL_ANY_NULL_PTR3(pst_hmac_vap, pst_hmac_user, pst_priv_req)) {
        OAM_ERROR_LOG0(0, OAM_SF_AMPDU, "{hmac_mgmt_tx_ampdu_end::param null}");
        return OAL_ERR_CODE_PTR_NULL;
    }
...
oal_uint32 hmac_tx_mgmt_send_event(mac_vap_stru *pst_vap, oal_netbuf_stru *pst_mgmt_frame, oal_uint16 us_frame_len)
{
    ...
    if (pst_vap == OAL_PTR_NULL || pst_mgmt_frame == OAL_PTR_NULL) {
        OAM_ERROR_LOG0(0, OAM_SF_SCAN, "{hmac_tx_mgmt_send_event::param null}");
        return OAL_ERR_CODE_PTR_NULL;
    }
...
oal_uint32 hmac_ie_proc_vht_opern_ie(mac_vap_stru *pst_mac_vap, const oal_uint8 *puc_payload,
    mac_user_stru *pst_mac_user)
{
    ...
    if (pst_vht_hdl->uc_channel_width >= 1) {
        ...
        if (en_bandwidth != pst_mac_vap->st_channel.en_bandwidth) {
            if (mac_mib_get_VHTChannelWidthOptionImplemented(pst_mac_vap) == WLAN_MIB_VHT_SUPP_WIDTH_80) {
        ...
        OAM_INFO_LOG2(pst_mac_vap->uc_vap_id, OAM_SF_ANY,
            "{hmac_ie_proc_vht_opern_ie::pst_mac_user->en_bandwidth_cap[%d], en_bwcap_vap[%d].}",
...
OAL_STATIC oal_uint32 hmac_ie_check_proc_opmode_param(mac_user_stru *pst_mac_user,
    mac_opmode_notify_stru *pst_opmode_notify)
{
    ...
    if ((pst_mac_user->en_bandwidth_cap < pst_opmode_notify->bit_channel_width) ||
        ...
        OAM_WARNING_LOG4(pst_mac_user->uc_vap_id, OAM_SF_ANY,
            "{hmac_ie_check_proc_opmode_param::user cap over limit! en_bandwidth_cap = [%d], opmode bandwidth = [%d],\
            user spatial stream = [%d], opmode rx nss = [%d]!}\r\n",
    ...
    if (pst_opmode_notify->bit_rx_nss_type == 1) {
        if (pst_mac_user->st_vht_hdl.bit_num_bf_ant_supported < pst_opmode_notify->bit_rx_nss) {
            OAM_WARNING_LOG2(pst_mac_user->uc_vap_id, OAM_SF_ANY,
                "{hmac_ie_check_proc_opmode_param::bit_rx_nss is over limit!bit_num_bf_ant_supported = [%d],\
                bit_rx_nss = [%d]!}\r\n",
...
oal_uint32 hmac_ie_proc_opmode_field(mac_vap_stru *pst_mac_vap, mac_user_stru *pst_mac_user,
    mac_opmode_notify_stru *pst_opmode_notify, oal_uint8 uc_mgmt_frm_type)
{
    wlan_bw_cap_enum_uint8 en_bwcap_vap = 0; /* vap自身带宽能力 */
    wlan_bw_cap_enum_uint8 en_avail_bw; /* vap自身带宽能力 */
    mac_device_stru        *pst_mac_dev = OAL_PTR_NULL;

    if (OAL_UNLIKELY(OAL_ANY_NULL_PTR3(pst_mac_user, pst_opmode_notify, pst_mac_vap))) {
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "{hmac_ie_proc_opmode_field::param null}");
        return OAL_ERR_CODE_PTR_NULL;
    }

    if (hmac_ie_check_proc_opmode_param(pst_mac_user, pst_opmode_notify) == OAL_FAIL) {
        OAM_WARNING_LOG0(0, OAM_SF_ANY, "{hmac_ie_proc_opmode_field::hmac_ie_check_proc_opmode_param return fail}\r\n");
        return OAL_FAIL;
    }
    ...
    if (pst_opmode_notify->bit_rx_nss_type == 1) {
        OAM_INFO_LOG0(0, OAM_SF_ANY, "{hmac_ie_proc_opmode_field::pst_opmode_notify->bit_rx_nss_type == 1!}\r\n");
    ...
    if (pst_opmode_notify->bit_rx_nss != pst_mac_user->uc_avail_num_spatial_stream) {
        OAM_INFO_LOG2(pst_mac_vap->uc_vap_id, OAM_SF_ANY,
            "{hmac_ie_proc_opmode_field::pst_opmode_notify->bit_rx_nss = [%x], \
            pst_mac_user->uc_avail_num_spatial_stream = [%x]!}\r\n",
        ...
        OAM_INFO_LOG2(pst_mac_vap->uc_vap_id, OAM_SF_ANY, "{hmac_ie_proc_opmode_field::change rss. \
            pst_mac_vap->en_vap_rx_nss = [%x], pst_mac_user->uc_avail_num_spatial_stream = [%x]!}\r\n",
    ...
    if (pst_opmode_notify->bit_channel_width != pst_mac_user->en_avail_bandwidth) {
        OAM_INFO_LOG2(pst_mac_vap->uc_vap_id, OAM_SF_ANY, "{hmac_ie_proc_opmode_field::\
        ...
        OAM_INFO_LOG2(0, OAM_SF_ANY,"{hmac_ie_proc_opmode_field::change bandwidth. en_bwcap_vap = [%x], \
            pst_mac_user->en_avail_bandwidth = [%x]!}\r\n", en_bwcap_vap, pst_mac_user->en_avail_bandwidth);
    }
...
OAL_STATIC oal_uint32 hmac_sa_query_del_user(mac_vap_stru *pst_mac_vap, hmac_user_stru *pst_hmac_user)
{
    ...
    switch (en_vap_mode) {
        ...
        case WLAN_VAP_MODE_BSS_STA:
        {
            ...
            hmac_sta_handle_disassoc_rsp(pst_hmac_vap, HMAC_REPORT_ACTION, DMAC_DISASOC_SA_QUERY_DEL_USER);
        }
        break;
        default:
            break;
                                             ...
                                             oal_bool_enum_uint8 en_is_protected)
{
    ...
    us_sa_query_len = hmac_encap_sa_query_req(pst_mac_vap, (oal_uint8 *)OAL_NETBUF_HEADER(pst_sa_query),
 ...
 */
uint32_t hmac_pmf_check_err_code(mac_user_stru *user_base_info, oal_bool_enum_uint8 is_protected,
    const uint8_t *mac_hdr)
{
    ...
    if ((user_base_info->st_cap_info.bit_pmf_active == OAL_TRUE) &&
        (aim_err_code == OAL_TRUE || is_protected == OAL_FALSE)) {
        return OAL_SUCC;
    }
...
uint32_t hmac_sa_query_interval_timeout(void *arg)
{
    ...
    if (hmac_user->st_user_base_info.en_user_asoc_state != MAC_USER_STATE_ASSOC) {
        hmac_user->st_sa_query_info.ul_sa_query_count = 0;
        OAM_WARNING_LOG1(vap_id, OAM_SF_ANY, "{hmac_sa_query_interval_timeout_etc::USER UNEXPECTED STATE[%d].}",
                         hmac_user->st_user_base_info.en_user_asoc_state);
        return OAL_SUCC;
    }

    now = (uint32_t)OAL_TIME_GET_STAMP_MS();
    passed = (uint32_t)OAL_TIME_GET_RUNTIME(hmac_user->st_sa_query_info.ul_sa_query_start_time, now);
    /* SA Query总时间超时 */
    if (passed >= mac_mib_get_dot11AssociationSAQueryMaximumTimeout(&hmac_vap->st_vap_base_info)) {
        /* change state & ul_sa_query_count=0 */
        OAM_WARNING_LOG0(vap_id, OAM_SF_ANY, "{hmac_sa_query_interval_timeout::Deleting user.}");
        relt = hmac_sa_query_del_user(&(hmac_vap->st_vap_base_info), hmac_user);
        if (relt != OAL_SUCC) {
            OAM_ERROR_LOG1(vap_id, OAM_SF_ANY, "{hmac_sa_query_interval_timeout::del user failed[%d].}", relt);
    ...
    hmac_user->st_sa_query_info.ul_sa_query_count += 1;
    OAM_WARNING_LOG1(vap_id, OAM_SF_ANY, "{hmac_sa_query_interval_timeout_etc::SA query, trans_id %d.}",
    ...
    /* 设置间隔定时器 */
    FRW_TIMER_CREATE_TIMER(&(hmac_user->st_sa_query_info.st_sa_query_interval_timer), hmac_sa_query_interval_timeout,
        retry_timeout, (void *)hmac_user, OAL_FALSE, OAM_MODULE_ID_HMAC, hmac_vap->st_vap_base_info.ul_core_id);

    relt = hmac_send_sa_query_req(&(hmac_vap->st_vap_base_info), hmac_user, hmac_user->st_sa_query_info.is_protected);
    if (relt != OAL_SUCC) {
        OAM_ERROR_LOG1(vap_id, OAM_SF_ANY, "{hmac_sa_query_interval_timeout_etc::send sa query failed[%d].}", relt);
...
uint32_t hmac_start_sa_query(mac_vap_stru *mac_vap, hmac_user_stru *hmac_user, oal_bool_enum_uint8 is_protected)
{
    ...
    /* 入参判断 */
    if ((mac_vap == NULL) || (hmac_user == NULL)) {
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "{hmac_start_sa_query::param null}");
        return OAL_ERR_CODE_PTR_NULL;
    }

    /* 判断vap有无pmf能力 */
    if (hmac_user->st_user_base_info.st_cap_info.bit_pmf_active != OAL_TRUE) {
        OAM_ERROR_LOG0(mac_vap->uc_vap_id, OAM_SF_ANY, "{hmac_start_sa_query::bit_pmf_active is down.}");
        return OAL_ERR_CODE_PMF_DISABLED;
    }

    /* 避免重复启动SA Query流程 */
    if (hmac_user->st_sa_query_info.ul_sa_query_count != 0) {
        OAM_INFO_LOG0(mac_vap->uc_vap_id, OAM_SF_ANY, "{hmac_start_sa_query::SA Query is already in process.}");
        return OAL_SUCC;
    }
    ...
    if (hmac_vap == NULL) {
        OAM_ERROR_LOG0(mac_vap->uc_vap_id, OAM_SF_ANY, "{hmac_start_sa_query::pst_hmac_vap null.}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    retry_timeout = (oal_uint16)mac_mib_get_dot11AssociationSAQueryRetryTimeout(mac_vap);
    /* 记录sa query流程开始时间,单位ms */
    hmac_user->st_sa_query_info.ul_sa_query_start_time = (oal_uint32)OAL_TIME_GET_STAMP_MS();
    /* 获得初始trans_id */
    hmac_user->st_sa_query_info.us_sa_query_trans_id = (oal_uint16)OAL_TIME_GET_STAMP_MS();
    hmac_user->st_sa_query_info.ul_sa_query_count = 1;
    hmac_user->st_sa_query_info.is_protected = is_protected;

    OAM_WARNING_LOG1(mac_vap->uc_vap_id, OAM_SF_ANY, "{hmac_start_sa_query::SA query, trans_id %d.}",
                     hmac_user->st_sa_query_info.us_sa_query_trans_id);

    /* 设置间隔定时器 */
    FRW_TIMER_CREATE_TIMER(&(hmac_user->st_sa_query_info.st_sa_query_interval_timer),
    ...
    ret = hmac_send_sa_query_req(mac_vap, hmac_user, is_protected);
    if (ret != OAL_SUCC) {
        OAM_ERROR_LOG1(0, OAM_SF_ANY, "{hmac_start_sa_query::hmac_send_sa_query_req failed[%d].}", ret);
        return OAL_ERR_CODE_PMF_SA_QUERY_REQ_SEND_FAIL;
    }
    ...
    oal_bool_enum_uint8 en_is_protected)
{
    ...
    if (pst_mac_vap->en_vap_state == MAC_VAP_STATE_BUTT) {
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_AUTH,
            "{hmac_mgmt_send_deauth_frame:: vap has been down/del, vap_state[%d].}", pst_mac_vap->en_vap_state);
        return;
    }
    ...
    us_deauth_len = hmac_mgmt_encap_deauth(pst_mac_vap, (uint8_t *)OAL_NETBUF_HEADER(pst_deauth), puc_da, us_err_code);
    if (us_deauth_len == 0) {
    ...
    mac_vap_set_cb_tx_user_idx(pst_mac_vap, pst_tx_ctl, puc_da); /* 发送完成需要获取user结构体 */
    ...
    if (pst_mac_vap->en_vap_mode == WLAN_VAP_MODE_BSS_STA) {
        ...
        if (ul_ret != OAL_SUCC) {
            OAM_WARNING_LOG1(0, OAM_SF_AUTH,
                "{hmac_mgmt_send_deauth_frame::mac_vap_find_user_by_macaddr failed.err code[%u].}", ul_ret);
            return;
        }
        pst_hmac_vap = (hmac_vap_stru *)mac_res_get_hmac_vap(pst_mac_vap->uc_vap_id);
        if (pst_hmac_vap == OAL_PTR_NULL)
        {
            OAM_ERROR_LOG1(0, OAM_SF_AUTH, "{hmac_mgmt_send_deauth_frame::mac_res_get_hmac_vap fail.vap_id[%u]}",
                pst_mac_vap->uc_vap_id);
            return;
        }
        hmac_fsm_change_state(pst_hmac_vap, MAC_VAP_STATE_STA_FAKE_UP);
    ...
    oal_bool_enum_uint8 en_is_protected)
{
    ...
    if (pst_mac_vap == OAL_PTR_NULL || puc_da == OAL_PTR_NULL) {
        OAM_ERROR_LOG0(0, OAM_SF_ASSOC, "{hmac_mgmt_send_disassoc_frame::param null}");
        return;
    }

    if (pst_mac_vap->en_vap_state == MAC_VAP_STATE_BUTT) {
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_ASSOC,
            "{hmac_mgmt_send_disassoc_frame:: vap has been down/del, vap_state[%d].}", pst_mac_vap->en_vap_state);
        return;
    }
    ...
    OAM_WARNING_LOG4(pst_mac_vap->uc_vap_id, OAM_SF_ASSOC,
        "{hmac_mgmt_send_disassoc_frame::Because of err_code[%d], send disassoc frame to dest addr, \
        da[%2x:xx:xx:xx:%2x:%2x].}", us_err_code, puc_da[0], puc_da[4], puc_da[5]);
    ...
    if (pst_mac_vap->en_vap_mode == WLAN_VAP_MODE_BSS_STA) {
        ...
        if (pst_hmac_vap == OAL_PTR_NULL) {
            OAM_ERROR_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_ASSOC,
                "{hmac_mgmt_send_disassoc_frame:: mac_res_get_hmac_vap fail.vap_id:%d}", pst_mac_vap->uc_vap_id);
            oal_netbuf_free(pst_disassoc);
            return;
        }
...
oal_void hmac_mgmt_update_11ntxbf_cap(oal_uint8 *puc_payload, hmac_user_stru *pst_hmac_user)
{
...
oal_void hmac_mgmt_update_histream_info(mac_vap_stru *pst_mac_vap, hmac_user_stru *pst_hmac_user,
                                        const oal_uint8 *puc_payload)
{
    mac_user_stru     *pst_mac_user;
    oal_uint32        ul_ret;
    ...
    if (st_histream_info.en_hidevice_type != MAC_HIDEVICE_NONE) {
...
oal_void hmac_set_user_protocol_mode(mac_vap_stru *pst_mac_vap, hmac_user_stru *pst_hmac_user)
{
    ...
    if (((pst_mac_user->en_protocol_mode == WLAN_LEGACY_11B_MODE) &&
        ...
        if (memcpy_s((oal_void *)&(pst_hmac_user->st_op_rates), sizeof(pst_hmac_user->st_op_rates),
...
oal_void hmac_user_init_rates(hmac_user_stru *pst_hmac_user, mac_vap_stru *pst_mac_vap)
{
    ...
    mac_rate_stru *pst_avail_op_rates = OAL_PTR_NULL;

    memset_s(pst_hmac_user->st_op_rates.auc_rs_rates, WLAN_MAX_SUPP_RATES, 0, WLAN_MAX_SUPP_RATES);
    ...
    for (uc_rate_idx = 0; uc_rate_idx < pst_avail_op_rates->uc_rs_nrates; uc_rate_idx++) {
        pst_avail_op_rates->auc_rs_rates[uc_rate_idx] =
            pst_mac_vap->st_curr_sup_rates.st_rate.ast_rs_rates[uc_rate_idx].uc_mac_rate;
    }
}

oal_uint8 hmac_add_user_rates(hmac_user_stru *pst_hmac_user, oal_uint8 uc_rates_cnt, const oal_uint8 *puc_rates)
{
    ...
    if (memcpy_s(&(pst_hmac_user->st_op_rates.auc_rs_rates[pst_hmac_user->st_op_rates.uc_rs_nrates]),
                 WLAN_MAX_SUPP_RATES, puc_rates, uc_rates_cnt) != EOK) {
...
void hmac_rx_sa_query_req(hmac_vap_stru *pst_hmac_vap, oal_netbuf_stru *pst_netbuf, oal_bool_enum_uint8 en_is_protected)
{
    ...
    if ((pst_hmac_vap == OAL_PTR_NULL) || (pst_netbuf == OAL_PTR_NULL)) {
        OAM_ERROR_LOG0(0, OAM_SF_RX, "{hmac_rx_sa_query_req::null param}");
        return;
    }
    ...
    oal_bool_enum_uint8 en_is_protected)
{
    ...
    oal_uint8         *puc_sa = OAL_PTR_NULL;
    hmac_user_stru    *pst_hmac_user = OAL_PTR_NULL;
    oal_uint16        *pus_trans_id = OAL_PTR_NULL;
    mac_sa_query_stru *pst_sa_query_info = OAL_PTR_NULL;

    if ((pst_hmac_vap == OAL_PTR_NULL) || (pst_netbuf == OAL_PTR_NULL)) {
        OAM_ERROR_LOG0(0, OAM_SF_AMPDU, "{hmac_rx_sa_query_rsp::param null}");
        return ;
    }
    ...
    pst_hmac_user = mac_vap_get_hmac_user_by_addr(&pst_hmac_vap->st_vap_base_info, puc_sa);
    if (pst_hmac_user == OAL_PTR_NULL) {
        OAM_ERROR_LOG0(0, OAM_SF_AMPDU, "{hmac_rx_sa_query_rsp::pst_hmac_user null.}");
        return ;
    }

    /* 如果该用户的管理帧加密属性不一致，丢弃该报文 */
    if (en_is_protected != pst_hmac_user->st_user_base_info.st_cap_info.bit_pmf_active) {
        OAM_ERROR_LOG0(0, OAM_SF_AMPDU, "{hmac_rx_sa_query_rsp::PMF check failed.}");
        return ;
    }
...
oal_uint32 hmac_mgmt_rx_opmode_notify_frame(hmac_vap_stru *pst_hmac_vap, oal_netbuf_stru *pst_netbuf)
{
    ...
    if ((pst_hmac_vap == OAL_PTR_NULL) || (pst_netbuf == OAL_PTR_NULL)) {
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "{hmac_mgmt_rx_opmode_notify_frame::param null}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    ul_ret = hmac_ie_proc_opmode_field(&(pst_hmac_vap->st_vap_base_info), pst_mac_user, pst_opmode_notify,
        uc_mgmt_frm_type);
...
oal_void hmac_rx_notify_channel_width(mac_vap_stru *pst_mac_vap, oal_netbuf_stru *pst_netbuf)
{
    ...
    /* 获取发送端地址 */
    mac_get_address2_host((oal_uint8 *)pst_rx_ctrl->st_rx_info.pul_mac_hdr_start_addr,
        auc_sta_addr, sizeof(auc_sta_addr));
    ...
    if (pst_mac_vap->en_vap_mode == WLAN_VAP_MODE_BSS_STA) {
        ...
        if (pst_hmac_user->ul_receive_ncw_cnt == 0) {
        ...
        if (pst_hmac_user->ul_receive_ncw_cnt >= HMAC_RECEIVE_NCW_THRED_CNT) {
            ...
            ul_timestamp_diff = OAL_TIME_GET_RUNTIME(pst_hmac_user->ul_ncw_start_time,
                (oal_uint32)OAL_TIME_GET_HIGH_PRECISION_MS());//lint !e666 !e573
            if ((ul_timestamp_diff <= HMAC_NCW_INHIBIT_THRED_TIME) &&
                (pst_hmac_user->st_user_base_info.uc_drop_ncw != OAL_TRUE)) {
                OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_2040,
                    "{hmac_rx_notify_channel_width::timediff[%lu]ms,drop_ncw}", ul_timestamp_diff);
                pst_hmac_user->st_user_base_info.uc_drop_ncw = OAL_TRUE;
            }
        }
    }
...
oal_void hmac_vap_add_proxysta_vap(mac_vap_stru *pst_mac_vap, oal_uint8 *puc_msg)
{
    ...
    if ((pst_mac_vap->st_vap_proxysta.en_is_proxysta == OAL_TRUE) &&
        ...
        if (memcpy_s(pst_event->auc_event_data, sizeof(pst_event->auc_event_data),
            pst_mac_cfg_set_oma, OAL_SIZEOF(mac_cfg_set_oma_param_stru)) != EOK) {
            OAM_ERROR_LOG0(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_SCAN,
                "hmac_vap_add_proxysta_vap::memcpy_s fail!");
            FRW_EVENT_FREE(pst_event_mem);
            return ;
        }
                                ...
                                oal_long          l_freq)
{
    ...
    FRW_EVENT_HDR_INIT(&(pst_event->st_event_hdr),
        FRW_EVENT_TYPE_HOST_CTX, HMAC_HOST_CTX_EVENT_SUB_TYPE_RX_MGMT,
        OAL_SIZEOF(hmac_rx_mgmt_event_stru), FRW_EVENT_PIPELINE_STAGE_0,
    ...
    l_ret += memcpy_s(pst_mgmt_frame->ac_name, OAL_IF_NAME_SIZE, pst_hmac_vap->pst_net_device->name,
        OAL_IF_NAME_SIZE);
#ifdef _PRE_WLAN_FEATURE_P2P
    ...
    if (!IS_LEGACY_VAP(pst_mac_vap)) {
        ...
        if (oal_compare_mac_addr(pst_frame_hdr->auc_address1,
            ...
            /* 第二个net dev槽 */
            l_ret += memcpy_s(pst_mgmt_frame->ac_name, OAL_IF_NAME_SIZE, pst_hmac_vap->pst_p2p0_net_device->name,
                OAL_IF_NAME_SIZE);
            ...
            l_ret += memcpy_s(pst_mgmt_frame->ac_name, OAL_IF_NAME_SIZE, pst_hmac_vap->pst_net_device->name,
                OAL_IF_NAME_SIZE);
        } else if ((uc_hal_vap_id == 14) && (pst_frame_hdr->st_frame_control.bit_sub_type == WLAN_PROBE_REQ) &&
    ...
    /* 分发事件 */
    if (frw_event_dispatch_event(pst_event_mem) != OAL_SUCC) {
        oal_free(pst_mgmt_data);
    }
    FRW_EVENT_FREE(pst_event_mem);
}

OAL_STATIC void hmac_wpas_set_tx_ctl_info(mac_tx_ctl_stru *tx_ctl, mac_mgmt_frame_stru *mgmt_tx)
{
    tx_ctl->us_mpdu_len       = mgmt_tx->us_len;    /* dmac发送需要的mpdu长度 */
    tx_ctl->us_tx_user_idx    = 0xF;                /* 发送完成需要获取user结构体 */
    tx_ctl->bit_need_rsp      = OAL_TRUE;
    tx_ctl->bit_is_vipframe   = OAL_TRUE;
    tx_ctl->bit_is_needretry  = OAL_TRUE;
    tx_ctl->bit_mgmt_frame_id = mgmt_tx->mgmt_frame_id;
}

oal_uint32 hmac_wpas_mgmt_tx(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, const oal_uint8 *puc_param)
{
    ...
    if (pst_mac_vap == OAL_PTR_NULL || puc_param == OAL_PTR_NULL) {
        OAM_ERROR_LOG0(0, OAM_SF_P2P, "{hmac_wpas_mgmt_tx::param null}");
        return OAL_FAIL;
    }
    ...
    /* 填充netbuf */
    if (memcpy_s((oal_uint8 *)OAL_NETBUF_HEADER(pst_netbuf_mgmt_tx), pst_mgmt_tx->us_len,
                 pst_mgmt_tx->puc_frame, pst_mgmt_tx->us_len) != EOK) {
        OAM_ERROR_LOG0(0, OAM_SF_P2P, "hmac_wpas_mgmt_tx::memcpy fail!");
    ...
    hmac_wpas_set_tx_ctl_info(pst_tx_ctl, pst_mgmt_tx);
...
#if defined(_PRE_WLAN_FEATURE_HS20) || defined(_PRE_WLAN_FEATURE_P2P) || defined(_PRE_WLAN_FEATURE_SAE)
...
oal_uint32 hmac_proc_ht_cap_ie(mac_vap_stru *pst_mac_vap, mac_user_stru *pst_mac_user, oal_uint8 *puc_ht_cap_ie)
{
    ...
    if ((pst_mac_vap == OAL_PTR_NULL) ||
        (pst_mac_user == OAL_PTR_NULL) ||
        (puc_ht_cap_ie == OAL_PTR_NULL)) {
        OAM_WARNING_LOG0(0, OAM_SF_ROAM, "{hmac_proc_ht_cap_ie::PARAM NULL!}");
        return OAL_ERR_CODE_PTR_NULL;
    }
...
oal_uint32 hmac_proc_vht_cap_ie(mac_vap_stru *pst_mac_vap, hmac_user_stru *pst_hmac_user, oal_uint8 *puc_vht_cap_ie)
{
    ...
    if ((pst_mac_vap == OAL_PTR_NULL) || (pst_hmac_user == OAL_PTR_NULL) || (puc_vht_cap_ie == OAL_PTR_NULL)) {
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "{hmac_proc_vht_cap_ie::param null}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    l_ret += memcpy_s(&(pst_mac_vht_hdl->st_rx_max_mcs_map), OAL_SIZEOF(mac_rx_max_mcs_map_stru),
                      &us_rx_mcs_map, OAL_SIZEOF(uint16_t));
    ...
    l_ret += memcpy_s(&(pst_mac_vht_hdl->st_tx_max_mcs_map), OAL_SIZEOF(mac_tx_max_mcs_map_stru),
                      &us_tx_mcs_map, OAL_SIZEOF(uint16_t));
    if (l_ret != EOK) {
