...
struct tcp_list_node *hmac_tcp_opt_find_oldest_node(hmac_vap_stru *pst_hmac_vap, hcc_chan_type dir)
{
    ...
    OAL_DLIST_SEARCH_FOR_EACH(pst_entry, &tmp_list->tcp_list)
    {
        ...
        if (oal_time_before_eq((unsigned long)node->ul_last_ts, (unsigned long)oldest_time) == OAL_TRUE) {
            oldest_time = node->ul_last_ts;
            oldest_node = node;
            OAM_INFO_LOG3(0, OAM_SF_ANY, "{dir:%d find last_ts %ld   oldest_time %ld.}", dir, node->ul_last_ts,
                oldest_time);
        }
    }
    if (oldest_node != NULL) {
...
{
    oal_uint16 dir_index;
    oal_uint16 tcp_index;
    oal_uint16 tcp_queue_index;

    OAL_BUG_ON(pst_hmac_vap == OAL_PTR_NULL);
    ...
    for (dir_index = 0; dir_index < HCC_DIR_COUNT; dir_index++) {
        for (tcp_index = 0; tcp_index < HMAC_TCP_STREAM; tcp_index++) {
            oal_spin_lock_init(&pst_hmac_vap->st_hamc_tcp_ack[dir_index].hmac_tcp_ack.hmac_tcp_ack_lock);
            oal_netbuf_list_head_init(
                &pst_hmac_vap->st_hamc_tcp_ack[dir_index].hmac_tcp_ack.hcc_ack_queue[tcp_index]);
            pst_hmac_vap->st_hamc_tcp_ack[dir_index].hmac_tcp_ack_list.tcp_pool[tcp_index].ul_used = 0;
            pst_hmac_vap->st_hamc_tcp_ack[dir_index].hmac_tcp_ack_list.tcp_pool[tcp_index].ul_index = tcp_index;
            pst_hmac_vap->st_hamc_tcp_ack[dir_index].hmac_tcp_ack_list.tcp_pool[tcp_index].ul_last_ts = jiffies;
        }
        for (tcp_queue_index = 0; tcp_queue_index < HMAC_TCP_OPT_QUEUE_BUTT; tcp_queue_index++) {
            oal_spin_lock_init(&pst_hmac_vap->st_hamc_tcp_ack[dir_index].data_queue_lock[tcp_queue_index]);
            oal_netbuf_head_init(&pst_hmac_vap->st_hamc_tcp_ack[dir_index].data_queue[tcp_queue_index]);
        }
        oal_dlist_init_head(&(pst_hmac_vap->st_hamc_tcp_ack[dir_index].hmac_tcp_ack_list.tcp_list));
        pst_hmac_vap->st_hamc_tcp_ack[dir_index].hmac_tcp_ack_list.ul_free_count = HMAC_TCP_STREAM;
                                         ...
                                         oal_uint16 us_index)
{
    ...
    l_ret += memcpy_s(&tcp_ack_no, sizeof(tcp_ack_no), &pst_tcp_hdr->ul_acknum, sizeof(oal_uint32));
    ...
    l_ret += memcpy_s(&tmp_tcp_ack_no[us_index], sizeof(oal_uint32), &pst_tcp_hdr->ul_acknum, sizeof(oal_uint32));
    if (l_ret != EOK) {
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "{hmac_tcp_opt_get_tcp_ack_type::memcpy_s failed !");
        return OAL_FAIL;
    }
    /* 赋值tcp_cb */
...
oal_uint32  hmac_tcp_opt_rx_get_tcp_ack(oal_netbuf_stru *skb, hmac_vap_stru *pst_hmac_vap, oal_uint16 *p_us_index,
    oal_uint8 dir)
{
    oal_ip_header_stru  *pst_ip_hdr  = OAL_PTR_NULL;
    oal_tcp_header_stru *pst_tcp_hdr = OAL_PTR_NULL;
...
#endif
    if (ul_tcp_type != TCP_ACK_FILTER_TYPE) {
        return OAL_FAIL;
    }
    return OAL_SUCC;
}
...
oal_uint32  hmac_tcp_opt_tx_get_tcp_ack(oal_netbuf_stru *skb, hmac_vap_stru *pst_hmac_vap, oal_uint16 *p_us_index,
    oal_uint8 dir)
{
    oal_ip_header_stru *pst_ip_hdr  = OAL_PTR_NULL;
    oal_tcp_header_stru *pst_tcp_hdr = OAL_PTR_NULL;
    oal_ether_header_stru *ether_hdr   = OAL_PTR_NULL;
    oal_uint32 ul_tcp_type;

    ether_hdr = (oal_ether_header_stru *)oal_netbuf_data(skb);
    if (hmac_judge_tx_netbuf_is_tcp_ack(ether_hdr) == OAL_FALSE) {
        /* not tcp ack data */
#ifdef _PRE_WLAN_TCP_OPT_DEBUG
    ...
    pst_ip_hdr = (oal_ip_header_stru *)(ether_hdr + 1);      /* 偏移一个snap，取ip头 */
    pst_tcp_hdr     = (oal_tcp_header_stru *)(pst_ip_hdr + 1);
    /* option4:flow index取不到时不过滤 */
    *p_us_index = (oal_uint16)hmac_tcp_opt_get_flow_index(pst_hmac_vap, pst_ip_hdr, pst_tcp_hdr, (hcc_chan_type)dir);
    if (*p_us_index == 0xFFFF) {
...
oal_uint16 hmac_tcp_opt_tcp_ack_list_filter(hmac_vap_stru *pst_hmac_vap, hmac_tcp_opt_queue type, hcc_chan_type dir,
...
oal_uint16 hmac_tcp_opt_rx_tcp_ack_filter(void *hmac_vap, hmac_tcp_opt_queue type, hcc_chan_type dir, void* data)
{
    ...
    OAL_BUG_ON(!hmac_vap);
    OAL_BUG_ON(!data);
    OAL_BUG_ON(type != HMAC_TCP_ACK_QUEUE);
    OAL_BUG_ON(dir != HCC_RX);
...
oal_uint32 hmac_tcp_opt_tcp_ack_filter(oal_netbuf_stru *skb, hmac_vap_stru *pst_hmac_vap, hcc_chan_type dir)
{
    ...
    oal_netbuf_stru *ack = OAL_PTR_NULL;
...
#ifdef _PRE_WLAN_TCP_OPT_DEBUG
    OAM_WARNING_LOG3(0, OAM_SF_ANY, "{dir:%d: stream:%d qlen:%u.}", dir, us_tcp_stream_index,
        oal_netbuf_list_len(hcc_ack_queue));
#endif
...
void hmac_tcp_ack_process_hcc_queue(hmac_vap_stru *pst_hmac_vap, hcc_chan_type dir, hmac_tcp_opt_queue type)
{
    oal_netbuf_head_stru st_head_t;
    oal_netbuf_head_stru *head = OAL_PTR_NULL;
    oal_netbuf_stru* pst_netbuf = OAL_PTR_NULL;
    ...
    if (pst_hmac_device == OAL_PTR_NULL) {
        ...
        if ((pst_hmac_device->sys_tcp_rx_ack_opt_enable == OAL_TRUE) ||
            ...
            for (uc_vap_idx = 0; uc_vap_idx < pst_hmac_device->pst_device_base_info->uc_vap_num; uc_vap_idx++) {
                pst_hmac_vap = (hmac_vap_stru *)mac_res_get_hmac_vap(
                    pst_hmac_device->pst_device_base_info->auc_vap_id[uc_vap_idx]);
...
oal_bool_enum_uint8 hmac_tcp_ack_need_schedule(void)
{
    ...
    if (pst_hmac_device == OAL_PTR_NULL) {
        ...
        if (pst_hmac_device->en_need_notify == OAL_TRUE) {
