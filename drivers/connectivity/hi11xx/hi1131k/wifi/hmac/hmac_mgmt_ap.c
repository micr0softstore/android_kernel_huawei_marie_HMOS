...
oal_void hmac_set_pmf_cap(mac_user_stru  *pst_mac_user, mac_vap_stru  *pst_mac_vap, const oal_uint8  *puc_rsn_ie)
{
    ...
    if ((en_ap_mfpc == OAL_FALSE) || (en_sta_mfpc == OAL_FALSE) || (en_ap_dot11RSNAActivated == OAL_FALSE)) {
...
OAL_STATIC void hmac_ap_set_user_auth_alg_num(uint16_t user_index, uint16_t auth_alg_num)
{
    mac_user_stru *mac_user = NULL;

    mac_user = mac_res_get_mac_user(user_index);
    if (mac_user == NULL) {
        return;
    }
    /* 当前最大WLAN_MIB_AUTH_ALG_SAE(3) */
    mac_user->st_key_info.auth_alg_num = (uint8_t)auth_alg_num;

    OAM_WARNING_LOG1(0, OAM_SF_AUTH, "{hmac_ap_set_user_auth_alg_num:: auth_alg_num=[%d]!}",
        mac_user->st_key_info.auth_alg_num);
}

/* AP收到ft/sae auth帧上报hostapd的处理 */
OAL_STATIC void hmac_ap_up_rx_auth_req_to_host(hmac_vap_stru *hmac_vap, oal_netbuf_stru *auth_req,
    uint8_t auth_alg_num)
{
    uint8_t  addr2[ETHER_ADDR_LEN] = {0};
    uint8_t  is_seq1;
    uint16_t auth_seq;
    uint16_t user_index = 0xffff;
    uint8_t  auth_resend = 0;
    uint32_t ret;

    OAM_INFO_LOG0(0, OAM_SF_AUTH, "{hmac_ap_up_rx_auth_req_to_host::recive sae auth.}");

    hmac_rx_mgmt_send_to_host(hmac_vap, auth_req);

    /* 获取STA的地址 */
    mac_get_address2_host(oal_netbuf_header(auth_req), addr2, sizeof(addr2));
    if (mac_addr_is_zero(addr2)) {
        OAM_WARNING_LOG4(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_AUTH,
                         "{hmac_ap_up_rx_auth_req_to_host::user mac:%02X:XX:XX:%02X:%02X:%02X is all 0 and invaild!}",
                         addr2[0], addr2[3], addr2[4], addr2[5]);
        return;
    }
    ...
    if (auth_seq > HMAC_AP_AUTH_SEQ3_WEP_COMPLETE) {
        OAM_WARNING_LOG1(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_AUTH,
                         "{hmac_ap_up_rx_auth_req_to_host::auth recieve invalid seq, auth seq [%d]}", auth_seq);
        return;
    }

    /* 获取用户idx */
    is_seq1 = (auth_seq == WLAN_AUTH_TRASACTION_NUM_ONE);
    ret = hmac_encap_auth_rsp_get_user_idx(&(hmac_vap->st_vap_base_info), addr2, sizeof(addr2),
                                           is_seq1, &auth_resend, &user_index);
    if (ret != OAL_SUCC) {
        if (ret == OAL_ERR_CODE_CONFIG_EXCEED_SPEC) {
            OAM_WARNING_LOG0(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_AUTH,
                "{hmac_ap_up_rx_auth_req_to_host::hmac_ap_get_user_idx fail, users exceed config spec!}");
        } else {
            OAM_WARNING_LOG0(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_AUTH,
                "{hmac_ap_up_rx_auth_req_to_host::hmac_ap_get_user_idx Err!}");
        }
    }

    hmac_ap_set_user_auth_alg_num(user_index, auth_alg_num);
    return;
}
#endif /* _PRE_WLAN_FEATURE_SAE */

OAL_STATIC uint32_t hmac_ap_rx_auth_sae_handle(hmac_vap_stru *hmac_vap, oal_netbuf_stru *auth_req,
    uint16_t auth_alg_num)
{
    OAM_WARNING_LOG1(0, OAM_SF_AUTH, "{hmac_ap_rx_auth_sae_handle::rx_auth_alg_num=[%d]}", auth_alg_num);

#ifdef _PRE_WLAN_FEATURE_SAE
    /* 如果是FT/SAE认证算法，上报hostapd */
    if (auth_alg_num == WLAN_MIB_AUTH_ALG_FT || auth_alg_num == WLAN_MIB_AUTH_ALG_SAE) {
        hmac_ap_up_rx_auth_req_to_host(hmac_vap, auth_req, auth_alg_num);
        return OAL_SUCC;
    } else if (auth_alg_num == WLAN_MIB_AUTH_ALG_OPEN_SYS &&
               g_mac_device_custom_cfg.forbit_open_auth) {
        OAM_WARNING_LOG0(0, OAM_SF_AUTH, "{hmac_ap_rx_auth_sae_handle::forbid open user to auth}");
        return OAL_SUCC;
    }
#endif
    return OAL_FAIL;
                                         ...
                                         dmac_wlan_crx_event_stru *pst_mgmt_rx_event)
{
    ...
    auth_alg_num = mac_get_auth_algo_num(pst_auth_req);
    if (hmac_ap_rx_auth_sae_handle(pst_hmac_vap, pst_auth_req, auth_alg_num) == OAL_SUCC) {
        return;
    }

    chtxt = pst_mgmt_rx_event->puc_chtxt;

    /* AP接收到STA发来的认证请求帧组相应的认证响应帧 */
    pst_auth_rsp = (oal_netbuf_stru *)OAL_MEM_NETBUF_ALLOC(OAL_NORMAL_NETBUF, WLAN_MEM_NETBUF_SIZE2,
        OAL_NETBUF_PRIORITY_MID);
    if (pst_auth_rsp == OAL_PTR_NULL) {
        OAM_ERROR_LOG0(0, OAM_SF_AUTH, "{hmac_ap_rx_auth_req::pst_auth_rsp null.}");
        return;
    }
    ...
    us_auth_rsp_len = hmac_encap_auth_rsp(&pst_hmac_vap->st_vap_base_info, pst_auth_rsp, pst_auth_req, chtxt);
    if (us_auth_rsp_len == 0) {
        oal_netbuf_free(pst_auth_rsp);
        OAM_ERROR_LOG0(0, OAM_SF_AUTH, "{hmac_ap_rx_auth_req::us_auth_rsp_len is 0.}");
        return;
    }
    ...
    /* 抛事件给dmac发送认证帧 */
    if (hmac_tx_mgmt_send_event(&pst_hmac_vap->st_vap_base_info, pst_auth_rsp, us_auth_rsp_len) != OAL_SUCC) {
        oal_netbuf_free(pst_auth_rsp);
        OAM_WARNING_LOG0(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_AUTH, "{hmac_ap_rx_auth_req::send failed.}");
    }
}
    ...
    oal_uint32                      ul_payload_len)
{
    uint32_t ret;
    /* 处理ERP相关 */
    ret = hmac_ap_process_obss_erp_ie(pst_hmac_vap, puc_payload, ul_payload_len);
    if (ret != OAL_SUCC) {
        OAM_WARNING_LOG0(0, OAM_SF_AUTH, "hmac_ap_rx_obss_beacon::hmac_ap_process_obss_erp_ie ret error");
    }

    /* 处理HT operation相关 */
    ret = hmac_ap_process_obss_ht_operation_ie(pst_hmac_vap, puc_payload, ul_payload_len);
    if (ret != OAL_SUCC) {
        OAM_WARNING_LOG0(0, OAM_SF_AUTH, "hmac_ap_rx_obss_beacon::hmac_ap_process_obss_ht_operation_ie ret error");
    }
    /* 更新AP中保护相关mib量 */
...
OAL_STATIC oal_uint32 hmac_ap_rx_deauth_req(hmac_vap_stru *pst_hmac_vap, oal_uint8 *puc_mac_hdr,
                                            oal_bool_enum_uint8 en_is_protected)
{
    ...
    OAM_WARNING_LOG4(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_AUTH,
        "{aput rx deauth frame, reason code = %d,sa[XX:XX:XX:%X:%X:%X]}", us_err_code, puc_sa[3], puc_sa[4], puc_sa[5]);
    ...
    if ((ETHER_IS_MULTICAST(puc_da) != OAL_TRUE) &&
        ...
        OAM_ERROR_LOG2(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_AUTH,
                       "{hmac_ap_rx_deauth_req::PMF check failed %d %d.}", en_is_protected,
                       pst_hmac_user->st_user_base_info.st_cap_info.bit_pmf_active);
        return OAL_FAIL;
    }
...
OAL_STATIC oal_uint32  hmac_user_check_update_exp_rate(hmac_user_stru *pst_hmac_user, const oal_uint8 *pst_params,
                                                       oal_uint8 *puc_erp_rates_num)
{
    ...
    oal_uint16                     *pus_msg_idx)
{
    ...
    *puc_num_rates = 0;
        ...
        *puc_num_rates = puc_sup_rates_ie[1];
        ...
        if (*puc_num_rates == 0) {
            ...
            *pus_msg_idx = us_msg_idx;
            OAM_ERROR_LOG0(0, OAM_SF_ANY, "{hmac_ap_up_update_sta_sup_rates::the sta's rates are not supported.}");
            return OAL_FAIL;
        }
    ...
    if (puc_ext_sup_rates_ie == OAL_PTR_NULL) {
        ...
        if (puc_ext_sup_rates_ie[0] == MAC_EID_XRATES && (pst_mac_vap->en_protocol > WLAN_LEGACY_11G_MODE)) {
...
OAL_STATIC uint32_t hmac_check_cap_and_pmkid(const uint8_t *ie_data, uint8_t *index,
    mac_status_code_enum_uint16 *status_code)
{
    ...
    /* 无RSN能力字段 */
    if (MAC_IE_REAMIN_LEN_IS_ENOUGH(2, *index, ie_data[1], 2) == OAL_FALSE) {
        OAM_WARNING_LOG0(0, OAM_SF_ASSOC, "{hmac_check_pmkid::no rsn cap.}");
        return OAL_SUCC;
    }
    ...
    *index += 2;
    ...
    if (MAC_IE_REAMIN_LEN_IS_ENOUGH(2, *index, ie_data[1], 2) == OAL_FALSE) {
        OAM_WARNING_LOG0(0, OAM_SF_ASSOC, "{hmac_check_pmkid::no rsn pmkid count.}");
        return OAL_SUCC;
    }

    pmkid_count = OAL_MAKE_WORD16(ie_data[*index], ie_data[*index + 1]);
    for (pmkid_idx = 0; pmkid_idx < pmkid_count; pmkid_idx++) {
        /* pmkid(16) */
        if (MAC_IE_REAMIN_LEN_IS_ENOUGH(2, *index, ie_data[1], 16) == OAL_FALSE) {
            *status_code = MAC_INVALID_PMKID;
            OAM_WARNING_LOG1(0, OAM_SF_ASSOC, "{hmac_check_pmkid::status_code[%d].}", MAC_INVALID_PMKID);
            return OAL_FAIL;
        }
        *index += 16;
    }
...
OAL_STATIC oal_uint32 hmac_check_assoc_req_security_cap_common(mac_vap_stru *pst_mac_vap,
    const oal_uint8 *puc_ie, oal_uint32 ul_msg_len, oal_uint8 uc_80211i_mode, oal_uint8 uc_offset,
    mac_status_code_enum_uint16 *pen_status_code)
{
    wlan_mib_ieee802dot11_stru  *pst_mib_info    = OAL_PTR_NULL;
    oal_uint8                    auth_policy;
    oal_uint8                    pcip_policy;
    oal_uint8                    grp_policy;
    ...
    /* 检测版本信息 */
    if (pst_mib_info->st_wlan_mib_rsna_cfg.ul_dot11RSNAConfigVersion !=
        OAL_MAKE_WORD16(puc_ie[uc_index], puc_ie[uc_index + 1])) {
        *pen_status_code = MAC_UNSUP_RSN_INFO_VER;
        OAM_ERROR_LOG1(0, OAM_SF_ASSOC, "{hmac_check_assoc_req_security_cap_common::unsup rsn version=%d.}",
                       OAL_MAKE_WORD16(puc_ie[uc_index], puc_ie[uc_index + 1]));
        return OAL_FAIL;
    }
    ...
    if (oal_memcmp(puc_oui, puc_ie + uc_index, MAC_OUI_LEN) != 0) {
        OAM_WARNING_LOG0(0, OAM_SF_ASSOC, "{hmac_check_assoc_req_security_cap_common::invalid OUI.}");
    }
    uc_index += MAC_OUI_LEN;
    grp_policy = puc_ie[uc_index++]; /* 获取组播密钥 */

    /* 获取单播加密套件 */
    pcip_policy = hmac_get_pcip_policy_auth(puc_ie + uc_index, &uc_len, puc_oui);
    uc_index += uc_len;

    /* 获取认证套件信息 */
    auth_policy = hmac_get_auth_policy_auth(puc_ie + uc_index, &uc_len, puc_oui);
    uc_index += uc_len;

    /* 成对密钥套件是否为组播密钥套件 */
    if (pcip_policy == WLAN_80211_CIPHER_SUITE_GROUP_CIPHER) {
        pcip_policy = grp_policy;
    }

    /* 检测组播套件 */
    if (pst_mib_info->st_wlan_mib_rsna_cfg.uc_dot11RSNAConfigGroupCipher != grp_policy) {
        *pen_status_code = MAC_INVALID_GRP_CIPHER;
        OAM_ERROR_LOG1(0, OAM_SF_ASSOC, "{hmac_check_assoc_req_security_cap_common::invalid group[%d].}", grp_policy);
        return OAL_FAIL;
    }

    /* 检测单播密钥套件 */
    if (hmac_check_pcip_policy(pst_mib_info, pcip_policy, uc_80211i_mode) != OAL_SUCC) {
        *pen_status_code = MAC_INVALID_PW_CIPHER;
        OAM_ERROR_LOG1(0, OAM_SF_ASSOC, "{hmac_check_assoc_req_security_cap_common::invalid pcip[%d].}", pcip_policy);
        return OAL_FAIL;
    }

    /* 检测认证套件 */
    if (mac_check_auth_policy(pst_mib_info, auth_policy) == OAL_FALSE) {
        *pen_status_code = MAC_INVALID_AKMP_CIPHER;
        OAM_ERROR_LOG1(0, OAM_SF_ASSOC, "{hmac_check_assoc_req_security_cap_common::invalid auth[%d].}", auth_policy);
        return OAL_FAIL;
    }
    ...
    if ((grp_policy == WLAN_80211_CIPHER_SUITE_CCMP) && (pcip_policy == WLAN_80211_CIPHER_SUITE_TKIP)) {
        *pen_status_code = MAC_CIPHER_REJ;
        OAM_ERROR_LOG0(0, OAM_SF_ASSOC, "{hmac_check_assoc_req_security_cap_common::group/pairwise cipher conflict.}");
        return OAL_FAIL;
    }

    if (hmac_check_cap_and_pmkid(puc_ie, &uc_index, pen_status_code) != OAL_SUCC) {
        return OAL_FAIL;
    }

    /* 更新mib 参数 */
    pst_mib_info->st_wlan_mib_rsna_cfg.uc_dot11RSNAGroupCipherRequested         = grp_policy;
    pst_mib_info->st_wlan_mib_rsna_cfg.uc_dot11RSNAPairwiseCipherRequested      = pcip_policy;
    pst_mib_info->st_wlan_mib_rsna_cfg.uc_dot11RSNAAuthenticationSuiteRequested = auth_policy;
...
OAL_STATIC oal_uint32 hmac_check_assoc_req_security_cap_authenticator(hmac_vap_stru *pst_hmac_vap,
    oal_uint8 *puc_mac_hdr, oal_uint8 *puc_payload, oal_uint32 ul_msg_len, hmac_user_stru *pst_hmac_user,
    mac_status_code_enum_uint16 *pen_status_code)
{
    ...
    if (pst_mib_info->st_wlan_mib_privacy.en_dot11RSNAActivated != OAL_TRUE) {
    ...
    if ((puc_rsn_ie == OAL_PTR_NULL) && (puc_wpa_ie == OAL_PTR_NULL)) {
        ...
        } else if (pst_hmac_vap->en_wps_active == OAL_TRUE) {
        ...
        if ((en_rsn_found == OAL_FALSE) && (pst_mac_vap->st_cap_flag.bit_wpa == OAL_TRUE)) {
...
OAL_STATIC void hmac_ap_update_sta_user_info_and_cap(mac_vap_stru *mac_vap, mac_user_stru *mac_user,
                                                     mac_status_code_enum_uint16 *status_code)
{
    uint32_t ret;

    ret = hmac_config_user_cap_syn(mac_vap, mac_user);
    if (ret != OAL_SUCC) {
        OAM_ERROR_LOG1(mac_user->uc_vap_id, OAM_SF_ASSOC,
                       "{hmac_ap_up_update_sta_user::hmac_config_usr_cap_syn failed[%d].}", ret);
    }
    ...
    if (ret != OAL_SUCC) {
        OAM_ERROR_LOG1(mac_user->uc_vap_id, OAM_SF_ASSOC,
                       "{hmac_ap_up_update_sta_user::usr_info_syn failed[%d].}", ret);
    }
    ...
    if (ret != OAL_SUCC) {
        *status_code = MAC_UNSPEC_FAIL;
        OAM_WARNING_LOG1(mac_vap->uc_vap_id, OAM_SF_ASSOC,
                         "{hmac_ap_up_update_sta_user::mac_user_set_avail_num_space_stream failed[%d].}", ret);
    }
}
    ...
    mac_status_code_enum_uint16    *pen_status_code)
{
    ...
    ***************************************************************************/
        ...
        if (st_cfg_ssid.uc_ssid_len != puc_ssid_ie[1]) {
            *pen_status_code = MAC_UNSPEC_FAIL;
            OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_ASSOC,
                             "{hmac_ap_up_update_sta_user::AP refuse STA assoc, ssid len mismatch, status_code=%d.}",
                             *pen_status_code);
            return OAL_FAIL;
        }

        if (oal_memcmp(&puc_ssid_ie[2], st_cfg_ssid.ac_ssid, st_cfg_ssid.uc_ssid_len) != 0) {
            *pen_status_code = MAC_UNSPEC_FAIL;
            OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_ASSOC,
    ...
    /* 当前用户已关联 */
    ul_rslt = hmac_ap_up_update_sta_sup_rates(puc_payload, pst_hmac_user, pen_status_code, ul_msg_len, us_msg_idx,
                                              &uc_num_rates, &us_rate_len);
    if (ul_rslt != OAL_SUCC) {
        OAM_WARNING_LOG2(pst_mac_vap->uc_vap_id, OAM_SF_ASSOC, "{hmac_ap_up_update_sta_user::AP refuse STA assoc,"
                         "update support rates failed, status_code[%d] ul_rslt[%d].}", *pen_status_code, ul_rslt);
        return ul_rslt;
    }

#if defined(_PRE_WLAN_FEATURE_WPA) || defined(_PRE_WLAN_FEATURE_WPA2)
    /* 检查接收到的ASOC REQ消息中的SECURITY参数.如出错,则返回对应的错误码 */
    ul_rslt = hmac_check_assoc_req_security_cap_authenticator(pst_hmac_vap, puc_mac_hdr, puc_payload, ul_msg_len,
                                                              pst_hmac_user, pen_status_code);
    if (ul_rslt != OAL_SUCC) {
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_ASSOC,
    ...
    /* 更新对应STA的legacy协议能力 */
    if (hmac_ap_up_update_legacy_capability(pst_hmac_vap, pst_hmac_user, us_cap_info) != OAL_SUCC) {
        OAM_WARNING_LOG0(0, OAM_SF_ASSOC, "hmac_ap_up_update_sta_user::hmac_ap_up_update_legacy_capability error!");
    }
    ...
    ul_ret = hmac_check_rsn_capability(pst_mac_vap, puc_rsn_ie, pen_status_code, pst_mac_user);
    if (ul_ret != OAL_SUCC) {
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_ASSOC,
    ...
    /* 获取用户与VAP协议模式交集 */
    mac_user_set_avail_protocol_mode(pst_mac_user,
        g_auc_avail_protocol_mode[pst_mac_vap->en_protocol][pst_mac_user->en_protocol_mode]);
    ...
    OAM_WARNING_LOG3(pst_mac_vap->uc_vap_id, OAM_SF_ASSOC,
                     ...
                     en_bwcap_ap, en_bandwidth_cap, pst_mac_user->en_cur_bandwidth);
    hmac_ap_update_sta_user_info_and_cap(pst_mac_vap, pst_mac_user, pen_status_code);
                                                ...
                                                oal_uint8      uc_mgmt_frm_type)
{
    ...
    if (pst_hmac_user->puc_assoc_req_ie_buff == OAL_PTR_NULL) {
        ...
        if (memcpy_s(pst_hmac_user->puc_assoc_req_ie_buff, ul_payload_size, puc_payload, ul_payload_len) != EOK) {
            OAM_ERROR_LOG0(pst_hmac_user->st_user_base_info.uc_vap_id, OAM_SF_ASSOC,
                           "hmac_ap_prepare_assoc_req::memcpy_s failed.");
            return OAL_FAIL;
        }
...
OAL_STATIC oal_uint32 hmac_ap_set_is_wavetest_sta(hmac_vap_stru *pst_hmac_vap, oal_uint8 *auc_sta_addr,
                                                  oal_uint8 uc_sta_addr_len)
{
    mac_device_stru             *pst_mac_device;

    pst_mac_device = mac_res_get_dev(pst_hmac_vap->st_vap_base_info.uc_device_id);
    if ((pst_mac_device == OAL_PTR_NULL) || (uc_sta_addr_len == 0)) {
    ...
    oal_uint32                      ul_payload_len)
{
    ...
    oal_uint8                       auc_sta_addr[WLAN_MAC_ADDR_LEN] = {0};
    mac_tx_ctl_stru                *pst_tx_ctl = OAL_PTR_NULL;
    ...
    if (ul_payload_len <= uc_frm_least_len) {
        OAM_WARNING_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ASSOC,
                         "hmac_ap_up_rx_asoc_req:assoc req len[%d] too short!", ul_payload_len);
        return OAL_FAIL;
    }

    mac_get_address2_host(puc_mac_hdr, auc_sta_addr, sizeof(auc_sta_addr));
    ...
    /* 是否符合触发SA query流程的条件 */
#if (_PRE_WLAN_FEATURE_PMF != _PRE_PMF_NOT_SUPPORT)
    if (OAL_ALL_TRUE_VALUE2(!pst_hmac_user->st_sa_query_info.st_sa_query_interval_timer.en_is_enabled,
        pst_hmac_user->st_user_base_info.st_cap_info.bit_pmf_active) &&
    ...
    if (en_status_code != MAC_REJECT_TEMP) {
        ...
        ul_rslt = hmac_ap_up_update_sta_user(pst_hmac_vap, puc_mac_hdr, puc_payload, ul_payload_len, pst_hmac_user,
                                             &en_status_code);
        ...
        if (en_status_code == MAC_SUCCESSFUL_STATUSCODE) {
            ...
            if (ul_rslt != OAL_SUCC) {
                OAM_ERROR_LOG2(pst_hmac_user->st_user_base_info.uc_vap_id, OAM_SF_ASSOC,
                    "{hmac_ap_up_rx_asoc_req::hmac_init_security fail[%d] status_code[%d].}", ul_rslt, MAC_UNSPEC_FAIL);
        ...
        if ((ul_rslt != OAL_SUCC) || (en_status_code != MAC_SUCCESSFUL_STATUSCODE)) {
            OAM_WARNING_LOG2(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_CFG,
                "{hmac_ap_up_rx_asoc_req::hmac_ap_up_update_sta_user fail rslt[%d] status_code[%d].", ul_rslt,
                en_status_code);
#ifdef _PRE_DEBUG_MODE_USER_TRACK
            mac_user_change_info_event(pst_hmac_user->st_user_base_info.auc_user_mac_addr,
                                       pst_hmac_vap->st_vap_base_info.uc_vap_id,
    ...
    pst_asoc_rsp = (oal_netbuf_stru *)OAL_MEM_NETBUF_ALLOC(OAL_NORMAL_NETBUF, WLAN_MEM_NETBUF_SIZE2,
                                                           OAL_NETBUF_PRIORITY_MID);
    ...
    if (en_status_code == MAC_SUCCESSFUL_STATUSCODE) {
...
#if (_PRE_MULTI_CORE_MODE_OFFLOAD_DMAC != _PRE_MULTI_CORE_MODE)
        hmac_ap_set_is_wavetest_sta(pst_hmac_vap, auc_sta_addr, sizeof(auc_sta_addr));
#endif

        /* AP检测STA成功，允许其关联成功 */
#ifdef _PRE_DEBUG_MODE_USER_TRACK
        mac_user_change_info_event(pst_hmac_user->st_user_base_info.auc_user_mac_addr,
            pst_hmac_vap->st_vap_base_info.uc_vap_id, pst_hmac_user->st_user_base_info.en_user_asoc_state,
            MAC_USER_STATE_ASSOC, OAM_MODULE_ID_HMAC, OAM_USER_INFO_CHANGE_TYPE_ASSOC_STATE);
#endif
        ...
        if (memcpy_s(st_80211_ucast_switch.auc_user_macaddr, WLAN_MAC_ADDR_LEN,
                     (const oal_void *)pst_hmac_user->st_user_base_info.auc_user_mac_addr,
                     OAL_SIZEOF(st_80211_ucast_switch.auc_user_macaddr)) != EOK) {
            OAM_ERROR_LOG0(pst_hmac_user->st_user_base_info.uc_vap_id, OAM_SF_ASSOC,
                           "hmac_ap_up_rx_asoc_req::memcpy_s failed!");
            oal_netbuf_free(pst_asoc_rsp);
            return OAL_FAIL;
        }
        hmac_config_80211_ucast_switch(&(pst_hmac_vap->st_vap_base_info), OAL_SIZEOF(st_80211_ucast_switch),
...
#ifdef _PRE_WLAN_FEATURE_SMPS
        hmac_smps_user_asoc_update(uc_user_prev_smpsmode, (&pst_hmac_user->st_user_base_info),
        ...
        OAM_WARNING_LOG1(pst_hmac_user->st_user_base_info.uc_vap_id, OAM_SF_ASSOC,
                         "{hmac_ap_up_rx_asoc_req::STA assoc AP OAL_SUCC! STA_indx=%d.}", us_user_idx);
#if (_PRE_MULTI_CORE_MODE_OFFLOAD_DMAC == _PRE_MULTI_CORE_MODE)
#if (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION)
...
void hmac_ap_report_and_del_info(hmac_vap_stru *hmac_vap, hmac_user_stru *hmac_user)
{
    uint32_t ret;
    ...
    if (ret != OAL_SUCC) {
        OAM_WARNING_LOG1(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ASSOC,
                         "{hmac_ap_up_rx_disasoc::hmac_user_del failed[%d].}", ret);
    }
}
    ...
    oal_bool_enum_uint8             en_is_protected)
{
    ...
    OAM_WARNING_LOG4(0, OAM_SF_ASSOC, "{hmac_ap_up_rx_disasoc::Because of err_code[%d], received disassoc"
        "frame from source addr %02x:xx:xx:xx:%02x:%02x.}", *((oal_uint16 *)(puc_mac_hdr + MAC_80211_FRAME_LEN)),
        puc_sa[0], puc_sa[4], puc_sa[5]);
    ...
    OAM_INFO_LOG0(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ASSOC, "{hmac_ap_up_rx_disasoc::ap rx disasoc req}");
    if (pst_hmac_user->st_user_base_info.en_user_asoc_state == MAC_USER_STATE_ASSOC) {
#if (_PRE_WLAN_FEATURE_PMF != _PRE_PMF_NOT_SUPPORT)
        /* 检查是否需要发送SA query request */
        ...
        mac_user_change_info_event(pst_hmac_user->st_user_base_info.auc_user_mac_addr,
            pst_hmac_vap->st_vap_base_info.uc_vap_id, pst_hmac_user->st_user_base_info.en_user_asoc_state,
            MAC_USER_STATE_AUTH_COMPLETE, OAM_MODULE_ID_HMAC, OAM_USER_INFO_CHANGE_TYPE_ASSOC_STATE);
#endif
        ...
        chr_exception_report(CHR_PLATFORM_EXCEPTION_EVENTID, CHR_SYSTEM_WIFI, CHR_LAYER_DRV,
            CHR_WIFI_DRV_EVENT_SOFTAP_PASSIVE_DISCONNECT, *((uint16_t *)(puc_mac_hdr + MAC_80211_FRAME_LEN)));
#endif
        hmac_ap_report_and_del_info(pst_hmac_vap, pst_hmac_user);
    }
...
OAL_STATIC oal_void  hmac_ap_up_rx_2040_coext(hmac_vap_stru *pst_hmac_vap, oal_netbuf_stru *pst_netbuf)
{
    ...
    if (
        ...
        (mac_mib_get_2040BSSCoexistenceManagementSupport(pst_mac_vap) == OAL_FALSE)) {
    ...
    if (OAL_UNLIKELY(pst_mac_device == OAL_PTR_NULL)) {
        OAM_ERROR_LOG0(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_2040,
                       "{hmac_ap_up_rx_2040_coext::pst_mac_device null.}");
        return;
    }
    ...
    if (puc_data[ul_index] == MAC_EID_2040_COEXT) {
        uc_coext_info = puc_data[ul_index + MAC_IE_HDR_LEN];//lint !e679
        ...
        ul_index += (MAC_IE_HDR_LEN + puc_data[ul_index + 1]);//lint !e679
    }

    /* 20/40 BSS Intolerant Channel Report IE */
    if (puc_data[ul_index] == MAC_EID_2040_INTOLCHREPORT) {
        oal_uint8           uc_len        = puc_data[ul_index + 1];//lint !e679
        ...
        OAM_INFO_LOG1(0, OAM_SF_2040, "{hmac_ap_up_rx_2040_coext::Chan Report with len=%d.}", uc_len);
...
OAL_STATIC oal_void  hmac_ap_process_obss_40mhz_intol(hmac_vap_stru *pst_hmac_vap, oal_uint8 *puc_payload,
                                                      oal_uint16 us_payload_len)
{
    mac_device_stru   *pst_mac_device;
...
OAL_STATIC oal_void  hmac_ap_up_rx_obss_beacon(hmac_vap_stru *pst_hmac_vap, oal_netbuf_stru *pst_netbuf)
{
    ...
    /* 获取帧体指针 */
    puc_payload = (oal_uint8 *)pst_rx_ctrl->st_rx_info.pul_mac_hdr_start_addr +
    ...
    OAM_WARNING_LOG4(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_SCAN,
        "{hmac_ap_up_rx_obss_beacon::uc_mgmt_frm_type[0x%04X],puc_payload[%p],us_payload_len[%d],band[%d].}",
        uc_mgmt_frm_type, (uintptr_t)puc_payload, us_payload_len, pst_hmac_vap->st_vap_base_info.st_channel.en_band);
    ...
    /* 处理HT operation相关 */
    if (hmac_ap_process_obss_ht_operation_ie(pst_hmac_vap, puc_payload, us_payload_len) != OAL_SUCC) {
        OAM_WARNING_LOG0(0, OAM_SF_SCAN, "hmac_ap_up_rx_obss_beacon::hmac_ap_process_obss_ht_operation_ie ret error!");
    }
...
OAL_STATIC oal_void  hmac_ap_up_rx_smps_frame(hmac_vap_stru *pst_hmac_vap, oal_netbuf_stru *pst_netbuf)
{
    ...
    mac_get_address2_host((oal_uint8 *)pst_rx_ctrl->st_rx_info.pul_mac_hdr_start_addr, auc_sta_addr, WLAN_MAC_ADDR_LEN);

    pst_mac_user = mac_vap_get_user_by_addr(&(pst_hmac_vap->st_vap_base_info), auc_sta_addr);
    if (pst_mac_user == OAL_PTR_NULL) {
        OAM_WARNING_LOG0(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_SMPS,
                         "{hmac_ap_up_rx_smps_frame::pst_mac_user null.}");
        return;
    }
    ...
    if ((puc_data[MAC_ACTION_OFFSET_ACTION + 1] & BIT0) == 0) {
        OAM_INFO_LOG0(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_SMPS,
                      "{hmac_ap_up_rx_smps_frame::user smps mode change.}");
...
OAL_STATIC oal_void  hmac_ap_up_rx_action(hmac_vap_stru *pst_hmac_vap, oal_netbuf_stru *pst_netbuf,
                                          oal_bool_enum_uint8 en_is_protected)
{
    dmac_rx_ctl_stru               *pst_rx_ctrl;
            ...
            switch (puc_data[MAC_ACTION_OFFSET_ACTION]) {
                ...
                case MAC_PUB_VENDOR_SPECIFIC:
                {
                    if (frame_body_len <= MAC_ACTION_CATEGORY_AND_CODE_LEN + MAC_OUI_LEN) {
                        OAM_WARNING_LOG1(0, OAM_SF_RX, "{hmac_action_category_public::frame_body_len %d.}",
                                         frame_body_len);
                        return;
                    }
#ifdef _PRE_WLAN_FEATURE_P2P
...
oal_uint32  hmac_ap_up_rx_mgmt(hmac_vap_stru *pst_hmac_vap, oal_void *p_param)
{
    ...
    if ((pst_hmac_vap == OAL_PTR_NULL) || (p_param == OAL_PTR_NULL)) {
        OAM_ERROR_LOG0(0, OAM_SF_RX, "{hmac_ap_up_rx_mgmt::param null}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    if (g_ul_print_wakeup_mgmt == OAL_TRUE) {
        ...
        OAM_WARNING_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_RX,
                         "{hmac_ap_up_rx_mgmt::wakeup mgmt type[0x%x]}", uc_mgmt_frm_type);
    }
#endif
    ...
    switch (uc_mgmt_frm_type) {
        ...
        case WLAN_FC0_SUBTYPE_PROBE_RSP:
            OAM_WARNING_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_SCAN,
                             "{hmac_ap_up_rx_mgmt::Beacon[0x%4X].}", uc_mgmt_frm_type);
        ...
        case WLAN_FC0_SUBTYPE_REASSOC_REQ:
            hmac_ap_up_rx_asoc_req(pst_hmac_vap, uc_mgmt_frm_type, puc_mac_hdr, ul_mac_hdr_len, puc_payload,
                                   (ul_msg_len - ul_mac_hdr_len));
            break;

        case WLAN_FC0_SUBTYPE_DISASSOC:
            hmac_ap_up_rx_disasoc(pst_hmac_vap, puc_mac_hdr, ul_mac_hdr_len, puc_payload, (ul_msg_len - ul_mac_hdr_len),
                                  en_is_protected);
            break;
...
OAL_STATIC oal_void  hmac_ap_store_network(mac_device_stru *pst_mac_device, oal_netbuf_stru *pst_netbuf)
{
    ...
    if ((en_already_present == OAL_FALSE) && (pst_bss_id_list->us_num_networks < WLAN_MAX_SCAN_BSS_PER_CH) &&
...
OAL_STATIC uint32_t hmac_ap_get_chan_idx_of_network(mac_vap_stru *mac_vap, uint8_t *payload,
                                                    uint16_t payload_len, uint8_t curr_chan_idx, uint8_t *chan_idx,
                                                    mac_sec_ch_off_enum_uint8 *sec_ch_offset)
{
    wlan_channel_band_enum_uint8 band = mac_vap->st_channel.en_band;
    uint8_t                      ch_idx = 0xFF;
    uint32_t                     ret;
    uint8_t                      *ie_data = OAL_PTR_NULL;

    if (payload_len <= (MAC_TIME_STAMP_LEN + MAC_BEACON_INTERVAL_LEN + MAC_CAP_INFO_LEN)) {
        *chan_idx = curr_chan_idx;
        OAM_WARNING_LOG1(0, OAM_SF_ANY, "{hmac_ap_get_chan_idx_of_network::payload_len[%d]}", payload_len);
        return OAL_SUCC;
    }
    ...
    ie_data = mac_find_ie(MAC_EID_HT_OPERATION, payload, payload_len);
    if (ie_data != OAL_PTR_NULL) {
        ret = mac_get_valid_channel_idx(band, mac_vap->uc_vap_id, ie_data[MAC_IE_HDR_LEN], &ch_idx);
        if (ret != OAL_SUCC) {
            OAM_WARNING_LOG0(mac_vap->uc_vap_id, OAM_SF_ANY,
                             "{hmac_ap_get_chan_idx_of_network::ht get channel idx failed!}");
            return ret;
        }
        *chan_idx = ch_idx;
        *sec_ch_offset = ie_data[MAC_IE_HDR_LEN + 1] & 0x03;
        return OAL_SUCC;
    }

    if (band == WLAN_BAND_2G) {
        ie_data = mac_find_ie(MAC_EID_DSPARMS, payload, payload_len);
        if (ie_data != OAL_PTR_NULL) {
            ret = mac_get_valid_channel_idx(band, mac_vap->uc_vap_id, ie_data[MAC_IE_HDR_LEN], &ch_idx);
            if (ret != OAL_SUCC) {
                OAM_WARNING_LOG0(mac_vap->uc_vap_id, OAM_SF_ANY,
                                 "{hmac_ap_get_chan_idx_of_network::ht get channel idx failed(2.4G)!}");
                return ret;
            }
            *chan_idx = ch_idx;
            return OAL_SUCC;
...
#ifdef _PRE_WLAN_FEATURE_20_40_80_COEXIST
    if (hmac_ap_update_2040_chan_info(pst_mac_device, pst_mac_vap, puc_payload, us_payload_len, uc_chan_idx,
        en_sec_ch_offset) != OAL_TRUE) {
        OAM_WARNING_LOG0(0, OAM_SF_RX, "hmac_ap_wait_start_rx_obss_beacon::hmac_ap_update_2040_chan_info may be error");
    }
#endif
}
...
oal_uint32  hmac_ap_wait_start_rx_mgmt(hmac_vap_stru *pst_hmac_vap, oal_void *p_param)
{
    ...
    if ((pst_hmac_vap == OAL_PTR_NULL) || (p_param == OAL_PTR_NULL)) {
        OAM_ERROR_LOG0(0, OAM_SF_RX, "{hmac_ap_wait_start_rx_mgmt::param null}");
        return OAL_ERR_CODE_PTR_NULL;
    }
...
oal_uint32  hmac_ap_wait_start_misc(hmac_vap_stru *pst_hmac_vap, oal_void *p_param)
{
    ...
    if (OAL_UNLIKELY((pst_hmac_vap == OAL_PTR_NULL) || (p_param == OAL_PTR_NULL))) {
        OAM_ERROR_LOG0(0, OAM_SF_RX, "{hmac_ap_wait_start_misc::param null}");
        return OAL_ERR_CODE_PTR_NULL;
    }
...
oal_uint32  hmac_ap_up_misc(hmac_vap_stru *pst_hmac_vap, oal_void *p_param)
{
    ...
    if (OAL_UNLIKELY((pst_hmac_vap == OAL_PTR_NULL) || (p_param == OAL_PTR_NULL))) {
        OAM_ERROR_LOG0(0, OAM_SF_RX, "{hmac_ap_up_misc::param null}");
        return OAL_ERR_CODE_PTR_NULL;
    }
