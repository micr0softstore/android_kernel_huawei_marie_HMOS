    ...
    dmac_msdu_proc_state_stru *msdu_state, mac_msdu_proc_status_enum_uint8 *proc_state)
{
    ...
    uint8_t             *buffer_data_addr = NULL; /* 指向netbuf数据域的指针 */
    uint16_t            offset;                   /* submsdu相对于data指针的偏移 */
...
/* 设置amsdu 最后一个 netbuf next指针为null */
...
{
    hmac_rx_ctl_stru *rx_ctrl = NULL;  /* 指向MPDU控制块信息的指针 */
    ...
    mac_ieee80211_frame_stru *pst_frame_hdr)
{
    hmac_rx_ctl_stru                *pst_rx_ctrl     = OAL_PTR_NULL;    /* 指向MPDU控制块信息的指针 */
    dmac_msdu_stru                  st_msdu;                            /* 保存解析出来的每一个MSDU */
    mac_msdu_proc_status_enum_uint8 en_process_state;   /* 解析AMSDU的状态 */
    dmac_msdu_proc_state_stru       st_msdu_state    = { 0 };             /* 记录MPDU的处理信息 */
...
#if (_PRE_MULTI_CORE_MODE_OFFLOAD_DMAC == _PRE_MULTI_CORE_MODE)
    /* 暂时规避mib_info 指针为空的问题，If mib info is null ptr,release the netbuf */
