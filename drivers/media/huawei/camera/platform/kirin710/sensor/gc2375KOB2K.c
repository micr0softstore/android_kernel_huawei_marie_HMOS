...
static bool s_gc2375KOB2K_power_on = false; //false for power down, ture for power up
struct mutex gc2375KOB2K_power_lock;
static struct platform_device *s_pdev = NULL;
static sensor_t *s_sensor = NULL;
static sensor_t s_gc2375KOB2K;
static char *sensor_dts_name = "GC2375KOB2K";

static struct sensor_power_setting gc2375KOB2K_power_setting[] = {
    {
...
static struct sensor_power_setting gc2375KOB2K_power_down_setting[] = {
    {
        .seq_type     = SENSOR_PWDN,
        .config_val   = SENSOR_GPIO_LOW,
        .sensor_index = SENSOR_INDEX_INVALID,
...
static int gc2375KOB2K_identify_id(
        hwsensor_intf_t* si)
{
    int32_t rc = 0;
    sensor_t* sensor = I2S(si);
...
static int gc2375KOB2K_csi_enable(hwsensor_intf_t* si)
{
    return 0;
}

static int gc2375KOB2K_csi_disable(hwsensor_intf_t* si)
{
    return 0;
}

static int
gc2375KOB2K_match_id(
         hwsensor_intf_t* si, void * data)
 {

     if (gc2375KOB2K_power_up(si) != 0) {
     ...
     if (!strncmp(sensor->board_info->name, sensor_dts_name, strlen(sensor_dts_name))) {
         module_id = gc2375KOB2K_identify_id(si);
         if (0 == module_id) {
...
static int gc2375KOB2K_config_power_on(hwsensor_intf_t* si)
{
    int ret = 0;
    mutex_lock(&gc2375KOB2K_power_lock);

    if (NULL == si || NULL == si->vtbl || NULL == si->vtbl->power_up)
    {
        cam_err("%s. si power_up is null", __func__);
        /*lint -e455 -esym(455,*)*/
        mutex_unlock(&gc2375KOB2K_power_lock);
        /*lint -e455 +esym(455,*)*/
        return -EINVAL;
    }

    if (!s_gc2375KOB2K_power_on){
        ret = si->vtbl->power_up(si);
        if (0 == ret) {
            s_gc2375KOB2K_power_on = true;
        } else {
...
static int gc2375KOB2K_config_power_off(hwsensor_intf_t* si)
{
    int ret = 0;
    mutex_lock(&gc2375KOB2K_power_lock);
...
static int gc2375KOB2K_config_match_id(hwsensor_intf_t* si, void *argp)
{
    int ret = 0;
        ...
        void  *argp)
{
    ...
    cam_debug("gc2375KOB2K cfgtype = %d",data->cfgtype);
    switch(data->cfgtype){
        case SEN_CONFIG_POWER_ON:
            ret = gc2375KOB2K_config_power_on(si);
            break;
        case SEN_CONFIG_POWER_OFF:
            ret = gc2375KOB2K_config_power_off(si);
            break;
        ...
        case SEN_CONFIG_MATCH_ID:
            ret = gc2375KOB2K_config_match_id(si, argp);
            break;
...
static hwsensor_vtbl_t s_gc2375KOB2K_vtbl =
{
    .get_name = gc2375KOB2K_get_name,
    .config = gc2375KOB2K_config,
    .power_up = gc2375KOB2K_power_up,
    .power_down = gc2375KOB2K_power_down,
    .match_id = gc2375KOB2K_match_id,
    .csi_enable = gc2375KOB2K_csi_enable,
    .csi_disable = gc2375KOB2K_csi_disable,
};
/* individual driver data for each device */

atomic_t volatile gc2375KOB2K_powered = ATOMIC_INIT(0);

static sensor_t s_gc2375KOB2K =
{
    .intf = { .vtbl = &s_gc2375KOB2K_vtbl, },
    .power_setting_array = {
        .size = ARRAY_SIZE(gc2375KOB2K_power_setting),
        .power_setting = gc2375KOB2K_power_setting,
    },
    .power_down_setting_array = {
        .size = ARRAY_SIZE(gc2375KOB2K_power_down_setting),
        .power_setting = gc2375KOB2K_power_down_setting,
    },
    .p_atpowercnt = &gc2375KOB2K_powered,
};

static const struct of_device_id
s_gc2375KOB2K_dt_match[] =
{
    {
        .compatible = "huawei,gc2375KOB2K",
        .data = &s_gc2375KOB2K.intf,
    },
    { } /* terminate list */
};

MODULE_DEVICE_TABLE(of, s_gc2375KOB2K_dt_match);
/* platform driver struct */
static int32_t gc2375KOB2K_platform_probe(struct platform_device* pdev);
static int32_t gc2375KOB2K_platform_remove(struct platform_device* pdev);
static struct platform_driver
s_gc2375KOB2K_driver =
{
    .probe = gc2375KOB2K_platform_probe,
    .remove = gc2375KOB2K_platform_remove,
    .driver =
    {
        .name = "huawei,gc2375KOB2K",
        .owner = THIS_MODULE,
        .of_match_table = s_gc2375KOB2K_dt_match,
    },
};


static int32_t
gc2375KOB2K_platform_probe(
        struct platform_device* pdev)
{
    ...
    mutex_init(&gc2375KOB2K_power_lock);
    np = pdev->dev.of_node;
    if (!np) {
    ...
    id = of_match_node(s_gc2375KOB2K_dt_match, np);
    if (!id) {
...
static int __init
gc2375KOB2K_init_module(void)
{
    return platform_driver_probe(&s_gc2375KOB2K_driver,
            gc2375KOB2K_platform_probe);
}

static void __exit
gc2375KOB2K_exit_module(void)
{
    platform_driver_unregister(&s_gc2375KOB2K_driver);
}

module_init(gc2375KOB2K_init_module);
module_exit(gc2375KOB2K_exit_module);
MODULE_DESCRIPTION("gc2375KOB2K");
