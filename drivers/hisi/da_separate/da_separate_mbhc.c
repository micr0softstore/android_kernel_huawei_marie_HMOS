...
#define HEADSET_PLUGIN 0x2
#define HEADSET_PLUGOUT 0x3
__attribute__ ((weak)) int fm_lan_notifier_call_chain(unsigned long val)
{
	UNUSED_PARAMETER(val);
	AUDIO_LOGI("fm_lan_notifier_call_chain: use weak\n");
	return 0;
}
enum jack_states {
	PLUG_NONE           = 0,
	...
	unsigned int irqs)
{
	...
	snd_soc_component_write(priv->codec, ANA_IRQ_REG0_REG, irqs);
...
static inline int read_hkadc_value(struct mbhc_data *priv)
{
	...
	priv->adc_voltage = hisi_adc_get_value(DA_SEPARATE_HKADC_CHN);
...
static_t void hs_jack_report(struct mbhc_data *priv)
{
	...
	case PLUG_NONE:
		jack_report = 0;
		fm_lan_notifier_call_chain(HEADSET_PLUGOUT);
		AUDIO_LOGI("plug out\n");
		break;
	case PLUG_HEADSET:
		jack_report = SND_JACK_HEADSET;
		fm_lan_notifier_call_chain(HEADSET_PLUGIN);
...
static void mbhc_reg_init(struct snd_soc_component *codec, struct mbhc_data *priv)
{
	...
	snd_soc_component_write(codec, DEB_CNT_HS_MIC_CFG_REG, 0x14);

	/* MBHC compare config 125mV 800mV 95% */
	snd_soc_component_write(codec, DA_SEPARATE_MBHD_VREF_CTRL, 0x9E);

	/* HSMICBIAS config voltage 2.7V */
	snd_soc_component_write(codec, DA_SEPARATE_HSMICB_CFG, 0x0B);

	/* clear HP MIXER channel select */
	snd_soc_component_write(codec, CODEC_ANA_RW20_REG, 0x0);

	/* config HP PGA gain to -20dB */
	snd_soc_component_write(codec, CODEC_ANA_RW21_REG, 0x0);

	/* Charge Pump clk pd, freq 768kHz */
	snd_soc_component_write(codec, DA_SEPARATE_CHARGE_PUMP_CLK_PD, 0x1A);
	...
	snd_soc_component_write(codec, ANA_IRQ_REG0_REG, IRQ_MSK_HS_ALL);
}
...
static void mbhc_lock_init(struct mbhc_data *priv)
{
	...
	wakeup_source_init(&priv->wake_lock, "da_separate_mbhc");
}
...
static void mbhc_priv_init(struct snd_soc_component *codec, struct mbhc_data *priv, const struct da_separate_mbhc_ops *ops)
{
	priv->codec = codec;
	mbhc_lock_init(priv);
...
int da_separate_mbhc_init(struct snd_soc_component *codec, struct da_separate_mbhc **mbhc, const struct da_separate_mbhc_ops *ops)
{
	int ret;
	struct mbhc_data *priv = NULL;
