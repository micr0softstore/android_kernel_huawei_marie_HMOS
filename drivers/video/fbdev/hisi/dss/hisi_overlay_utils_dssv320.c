...
int hisi_dss_aif_handler(struct dpu_fb_data_type *dpufd, dss_overlay_t *pov_req, dss_overlay_block_t *pov_h_block)
{
	int i = 0;
	int k = 0;
		...
		if ((pov_req->ovl_idx == DSS_OVL2) || (pov_req->ovl_idx == DSS_OVL3)) {
			if (layer && ((layer->need_cap & CAP_AFBCD) != CAP_AFBCD)) {
				...
				if ((layer->src_rect.w > layer->dst_rect.w) &&
					(layer->src_rect.w > get_panel_xres(dpufd))) {/*lint !e574 */
	...
	if (g_fpga_flag == 0) {
		if ((ovl_idx == DSS_OVL0) || (ovl_idx == DSS_OVL1)) {
			if (layer && (layer->need_cap & CAP_AFBCD)) {
				...
				if ((layer->src_rect.w > layer->dst_rect.w) &&
					(layer->src_rect.w > get_panel_xres(dpufd))) {/*lint !e574 */
					scfd_h = layer->src_rect.w * 100 / get_panel_xres(dpufd);/*lint !e573 */
				} else {
					scfd_h = 100;
				}
...
/*lint -e701 -e732 */
//only for csc8b
int hisi_dss_csc_config(struct dpu_fb_data_type *dpufd,
	dss_layer_t *layer, dss_wb_layer_t *wb_layer)
{
	...
	} else if ((chn_idx == DSS_RCHN_V1) && (!is_yuv(format))){ //v1, rgb format
		if (layer) {
	...
	char __iomem *ovl_base, dss_ovl_t *s_ovl, int ovl_idx)
{
	...
	if ((ovl_idx == DSS_OVL1) || (ovl_idx == DSS_OVL3)) {
		DPU_FB_ERR("ovl_idx not surport:%d ",ovl_idx);
		return;
	}
...
static int hisi_dss_arsr2p_write_coefs(struct dpu_fb_data_type *dpufd, bool enable_cmdlist,
...
static void hisi_dss_arsr2p_write_config_coefs(struct dpu_fb_data_type *dpufd, bool enable_cmdlist,
	char __iomem *addr, const int **scl_down, const int **scl_up, int row, int col)
{
	int ret = 0;

	ret = hisi_dss_arsr2p_write_coefs(dpufd, enable_cmdlist, addr, scl_down, row, col);
	if (ret < 0) {
		DPU_FB_ERR("Error to write COEF_SCL_DOWN coefficients.\n");
		return;
	}

	ret = hisi_dss_arsr2p_write_coefs(dpufd, enable_cmdlist, addr + ARSR2P_SCL_UP_OFFSET, scl_up, row, col);
	if (ret < 0) {
...
void hisi_dss_arsr2p_coef_on(struct dpu_fb_data_type *dpufd, bool enable_cmdlist)
{
	...
	/* COEFY_V COEFY_H */
	hisi_dss_arsr2p_write_config_coefs(dpufd, enable_cmdlist, coefy_v, (const int **)COEF_Y_SCL_DOWN_TAP6, (const int **)COEF_Y_SCL_UP_TAP6, ARSR2P_PHASE_NUM, ARSR2P_TAP6);
	hisi_dss_arsr2p_write_config_coefs(dpufd, enable_cmdlist, coefy_v + ARSR2P_COEF_H0_OFFSET, (const int **)COEF_Y_SCL_DOWN_TAP6, (const int **)COEF_Y_SCL_UP_TAP6, ARSR2P_PHASE_NUM, ARSR2P_TAP6);
	hisi_dss_arsr2p_write_config_coefs(dpufd, enable_cmdlist, coefy_v + ARSR2P_COEF_H1_OFFSET, (const int **)COEF_Y_SCL_DOWN_TAP6, (const int **)COEF_Y_SCL_UP_TAP6, ARSR2P_PHASE_NUM, ARSR2P_TAP6);

	/* COEFA_V COEFA_H */
	hisi_dss_arsr2p_write_config_coefs(dpufd, enable_cmdlist, coefa_v, (const int **)COEF_AUV_SCL_DOWN_TAP4, (const int **)COEF_AUV_SCL_UP_TAP4, ARSR2P_PHASE_NUM, ARSR2P_TAP4);
	hisi_dss_arsr2p_write_config_coefs(dpufd, enable_cmdlist, coefa_v + ARSR2P_COEF_H0_OFFSET, (const int **)COEF_AUV_SCL_DOWN_TAP4, (const int **)COEF_AUV_SCL_UP_TAP4, ARSR2P_PHASE_NUM, ARSR2P_TAP4);
	hisi_dss_arsr2p_write_config_coefs(dpufd, enable_cmdlist, coefa_v + ARSR2P_COEF_H1_OFFSET, (const int **)COEF_AUV_SCL_DOWN_TAP4, (const int **)COEF_AUV_SCL_UP_TAP4, ARSR2P_PHASE_NUM, ARSR2P_TAP4);

	/* COEFUV_V COEFUV_H */
	hisi_dss_arsr2p_write_config_coefs(dpufd, enable_cmdlist, coefuv_v, (const int **)COEF_AUV_SCL_DOWN_TAP4, (const int **)COEF_AUV_SCL_UP_TAP4, ARSR2P_PHASE_NUM, ARSR2P_TAP4);
	hisi_dss_arsr2p_write_config_coefs(dpufd, enable_cmdlist, coefuv_v + ARSR2P_COEF_H0_OFFSET, (const int **)COEF_AUV_SCL_DOWN_TAP4, (const int **)COEF_AUV_SCL_UP_TAP4, ARSR2P_PHASE_NUM, ARSR2P_TAP4);
	hisi_dss_arsr2p_write_config_coefs(dpufd, enable_cmdlist, coefuv_v + ARSR2P_COEF_H1_OFFSET, (const int **)COEF_AUV_SCL_DOWN_TAP4, (const int **)COEF_AUV_SCL_UP_TAP4, ARSR2P_PHASE_NUM, ARSR2P_TAP4);
}
...
static int hisi_dss_arsr2p_config_check_width(dss_rect_t *dest_rect, int source_width, bool hscl_en, bool vscl_en)
{
	...
	if ((source_width < ARSR2P_MIN_INPUT) || (dest_rect->w < ARSR2P_MIN_INPUT) ||
		...
		if ((!hscl_en) && (!vscl_en)) {
			//sharpen_en = false;
			DPU_FB_INFO("src_rect.w(%d) or dst_rect.w(%d) is smaller than 16 or larger than 2560, arsr2p bypass!\n",
...
int hisi_dss_arsr2p_config(struct dpu_fb_data_type *dpufd, dss_layer_t *layer, dss_rect_t *aligned_rect, bool rdma_stretch_enable)
{
	...
	do {
		...
		if ((src_rect.h > dst_rect.h) && (DSS_HEIGHT(dst_rect.h) != 0)) {
			...
			if (dst_rect.h != 0)
				iv_inc = (DSS_HEIGHT(src_rect.h) * ARSR2P_INC_FACTOR +
	...
	char __iomem *scl_lut_base, int coef_lut_idx)
{
	...
	ret = hisi_dss_scl_write_coefs(dpufd, enable_cmdlist, h0_y_addr, (const int **)coef_lut_tap6[coef_lut_idx], lut_tap_addr);
	if (ret < 0) {
		DPU_FB_ERR("Error to write H0_Y_COEF coefficients.\n");
	}

	lut_tap_addr.col = TAP5;
	ret = hisi_dss_scl_write_coefs(dpufd, enable_cmdlist, y_addr, (const int **)coef_lut_tap5[coef_lut_idx], lut_tap_addr);
	if (ret < 0) {
		DPU_FB_ERR("Error to write Y_COEF coefficients.\n");
	}

	lut_tap_addr.col = TAP4;
	ret = hisi_dss_scl_write_coefs(dpufd, enable_cmdlist, uv_addr, (const int **)coef_lut_tap4[coef_lut_idx], lut_tap_addr);
	if (ret < 0) {
...
void hisi_remove_mctl_mutex(struct dpu_fb_data_type *dpufd, int mctl_idx, uint32_t cmdlist_idxs)
{
	...
	for (i = 0; i < HISI_DSS_CMDLIST_MAX; i++) {
		if ((cmdlist_idxs_temp & 0x1) == 0x1) {
			...
			/* index 2/3/9/11 is invalid cmdlist channel */
			if ((i != 2) && (i != 3) && (i != 9) && (i != 11))
#endif
	...
	char __iomem *post_clip_base, dss_post_clip_t *s_post_clip, int chn_idx)
{
	...
	post_clip_base = dpufd->dss_base + g_dss_module_base[chn_idx][MODULE_POST_CLIP_ES];
	dpufd->set_reg(dpufd, post_clip_base + POST_CLIP_DISP_SIZE_ES, s_post_clip->disp_size, 32, 0);
	dpufd->set_reg(dpufd, post_clip_base + POST_CLIP_CTL_HRZ_ES, s_post_clip->clip_ctl_hrz, 32, 0);
	dpufd->set_reg(dpufd, post_clip_base + POST_CLIP_CTL_VRZ_ES, s_post_clip->clip_ctl_vrz, 32, 0);
	dpufd->set_reg(dpufd, post_clip_base + POST_CLIP_EN_ES, s_post_clip->ctl_clip_en, 32, 0);
}

static int hisi_dss_check_userdata_check(struct dpu_fb_data_type *dpufd,
	dss_overlay_t *pov_req, dss_overlay_block_t *pov_h_block_infos)
{
	...
	if ((dpufd->index != PRIMARY_PANEL_IDX) && (dpufd->index != AUXILIARY_PANEL_IDX)){
		DPU_FB_ERR("hisi fb%d is invalid!\n", dpufd->index);
		return -EINVAL;
	}
	return 0;
}
...
/*lint -e613*/
static int hisi_dss_check_wblayer_buff(struct dpu_fb_data_type *dpufd, dss_wb_layer_t *wb_layer)
{
	...
	dss_overlay_t *pov_req, dss_overlay_block_t *pov_h_block_infos)
{
	...
	if (hisi_dss_check_userdata_check(dpufd, pov_req, pov_h_block_infos) == -EINVAL) {
		return -EINVAL;
	}
...
static int hisi_dss_check_layer_par_base(struct dpu_fb_data_type *dpufd, dss_layer_t *layer)
{
	if (dpufd == NULL) {
...
/*lint -e613*/
static int hisi_dss_check_layer_buff(struct dpu_fb_data_type *dpufd, dss_layer_t *layer)
{
...
int hisi_dss_check_layer_par(struct dpu_fb_data_type *dpufd, dss_layer_t *layer)
{
	...
	/*lint -e838*/
	ret = hisi_dss_check_layer_par_base(dpufd, layer);
	if (ret <= 0) {
		return ret;
	}
	/*lint +e838*/
	...
	if (layer->need_cap & CAP_AFBCD) {
		if ((layer->img.afbc_header_stride == 0) || (layer->img.afbc_payload_stride == 0)
			...
			DPU_FB_ERR("fb%d, afbc_header_stride=%d, afbc_payload_stride=%d, mmbuf_size=%d is invalid!",
				dpufd->index, layer->img.afbc_header_stride,
...
{
	char __iomem *peri_crg_base = dpufd->peri_crg_base;
	char __iomem *pmctrl_base = dpufd->pmctrl_base;
	char __iomem *sctrl_base = dpufd->sctrl_base;
	char __iomem *media_crg_base = dpufd->media_crg_base;
	uint32_t ret;
...
/*static int hisi_dss_mmbuf_config(struct dpu_fb_data_type *dpufd, int ovl_idx,
	dss_layer_t *layer, uint32_t hfbcd_block_type, bool is_pixel_10bit)
{
	...
	if (mm_alloc_needed) {
		...
		dpufd->mmbuf_info->mm_size0_y8[chn_idx] = hfbcd_rect.right * layer->img.bpp * mmbuf_line_num;
		dpufd->mmbuf_info->mm_base0_y8[chn_idx] = hisi_dss_mmbuf_alloc(g_mmbuf_gen_pool,
			dpufd->mmbuf_info->mm_size0_y8[chn_idx]);
		dpufd->mmbuf_info->mm_size1_c8[chn_idx] = dpufd->mmbuf_info->mm_size0_y8[chn_idx] / 2;
		dpufd->mmbuf_info->mm_base1_c8[chn_idx] = hisi_dss_mmbuf_alloc(g_mmbuf_gen_pool,
		...
		if (is_pixel_10bit) {
			...
			dpufd->mmbuf_info->mm_base2_y2[chn_idx] = hisi_dss_mmbuf_alloc(g_mmbuf_gen_pool,
				dpufd->mmbuf_info->mm_size2_y2[chn_idx]);
			dpufd->mmbuf_info->mm_size3_c2[chn_idx] = dpufd->mmbuf_info->mm_size2_y2[chn_idx] / 2;
			dpufd->mmbuf_info->mm_base3_c2[chn_idx] = hisi_dss_mmbuf_alloc(g_mmbuf_gen_pool,
...
static uint32_t *hisi_dss_get_acm_lut_hue_table(struct dpu_fb_data_type *dpufd, uint8_t last_gamma_type)
{
...
static uint32_t *hisi_dss_get_acm_lut_sata_table(struct dpu_fb_data_type *dpufd, uint8_t last_gamma_type)
{
	struct dpu_panel_info *pinfo = &(dpufd->panel_info);
...
static uint32_t *hisi_dss_get_acm_lut_satr0_table(struct dpu_fb_data_type *dpufd, uint8_t last_gamma_type)
{
	struct dpu_panel_info *pinfo = &(dpufd->panel_info);
...
static uint32_t *hisi_dss_get_acm_lut_satr1_table(struct dpu_fb_data_type *dpufd, uint8_t last_gamma_type)
{
	struct dpu_panel_info *pinfo = &(dpufd->panel_info);
...
static uint32_t *hisi_dss_get_acm_lut_satr2_table(struct dpu_fb_data_type *dpufd, uint8_t last_gamma_type)
{
	struct dpu_panel_info *pinfo = &(dpufd->panel_info);
...
static uint32_t *hisi_dss_get_acm_lut_satr3_table(struct dpu_fb_data_type *dpufd, uint8_t last_gamma_type)
{
	struct dpu_panel_info *pinfo = &(dpufd->panel_info);
...
static uint32_t *hisi_dss_get_acm_lut_satr4_table(struct dpu_fb_data_type *dpufd, uint8_t last_gamma_type)
{
	struct dpu_panel_info *pinfo = &(dpufd->panel_info);
...
static uint32_t *hisi_dss_get_acm_lut_satr5_table(struct dpu_fb_data_type *dpufd, uint8_t last_gamma_type)
{
	struct dpu_panel_info *pinfo = &(dpufd->panel_info);
...
static uint32_t *hisi_dss_get_acm_lut_satr6_table(struct dpu_fb_data_type *dpufd, uint8_t last_gamma_type)
{
	struct dpu_panel_info *pinfo = &(dpufd->panel_info);
...
static uint32_t *hisi_dss_get_acm_lut_satr7_table(struct dpu_fb_data_type *dpufd, uint8_t last_gamma_type)
{
	struct dpu_panel_info *pinfo = &(dpufd->panel_info);
...
void hisi_dss_dpp_acm_gm_set_reg(struct dpu_fb_data_type *dpufd) {
	...
	if (1 == gamma_config_flag) {
		...
		local_acm_lut_hue_table = hisi_dss_get_acm_lut_hue_table(dpufd, last_gamma_type);
		local_acm_lut_sata_table = hisi_dss_get_acm_lut_sata_table(dpufd, last_gamma_type);
		local_acm_lut_satr0_table = hisi_dss_get_acm_lut_satr0_table(dpufd, last_gamma_type);
		local_acm_lut_satr1_table = hisi_dss_get_acm_lut_satr1_table(dpufd, last_gamma_type);
		local_acm_lut_satr2_table = hisi_dss_get_acm_lut_satr2_table(dpufd, last_gamma_type);
		local_acm_lut_satr3_table = hisi_dss_get_acm_lut_satr3_table(dpufd, last_gamma_type);
		local_acm_lut_satr4_table = hisi_dss_get_acm_lut_satr4_table(dpufd, last_gamma_type);
		local_acm_lut_satr5_table = hisi_dss_get_acm_lut_satr5_table(dpufd, last_gamma_type);
		local_acm_lut_satr6_table = hisi_dss_get_acm_lut_satr6_table(dpufd, last_gamma_type);
		local_acm_lut_satr7_table = hisi_dss_get_acm_lut_satr7_table(dpufd, last_gamma_type);
...
void hisi_dss_post_scf_set_reg(struct dpu_fb_data_type *dpufd,
...
int hisi_dss_post_scf_config(struct dpu_fb_data_type *dpufd, dss_overlay_t *pov_req)
{
	...
	uint32_t v_ratio = SCF_INC_FACTOR;
