...
static void get_hisi_cmdlist_config(char __iomem *cmdlist_base, uint32_t *cmdlist_idxs_temp,
	...
	dss_overlay_t *pov_req, uint32_t cmdlist_idxs)
{
	...
	if (g_chip_id == KIRIN710_KEYWEST_CHIP_ID) {
		hisi_mctl_ctl_clear(dpufd, mctl_idx);
#if defined(CONFIG_HISI_FB_V320)
	...
	get_hisi_cmdlist_config(cmdlist_base, &cmdlist_idxs_temp, &start_sel_temp);
	...
	hisi_cmdlist_config_mif_reset(dpufd, pov_req, cmdlist_idxs, mctl_idx);
